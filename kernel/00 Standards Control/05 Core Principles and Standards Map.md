...
2. Separation of concerns：知识、术语、系统设计、案例和表达产物各自承担不同职责。
...
8. Foundations remain complete：所选 `Profile Scope` 的应用重点不代表可以删除或压缩其基础知识。
9. Source-to-knowledge：外部来源先经过 claim extraction、综合和归属判断，再改变 canonical knowledge。
10. Expression separation：派生表达材料由 `Expression Layer Entry` 注册并独立于 canonical knowledge 保存。
11. No empty completion：空壳页面、占位链接和只有两三句的核心页面不算完成。
12. Continuous verification：每批内容都要执行链接、公式、渲染、来源、重复性和覆盖度检查。
13. State separation：任务、写作、表达、证据和学习状态不能互相替代。
...
23. Language contract：reader-facing language、canonical identity、显示顺序与例外边界由所选 `Language Contract` 登记，kernel 不固化具体语言。

## Standards Map

- `Read Sets` [[kernel/Read Sets/00 Read Sets Index|Read Sets Index]]：按任务、事件和执行阶段组合需要读取的 leaf modules。
- `01` [[kernel/01 Scope and Architecture Standard|Scope and Architecture Standard]]：通用范围边界、基础保全、logical architecture，以及由 `Profile Scope` 提供的具体 scope binding。
...
- `08` [[kernel/08 Metadata and Status Standard|Metadata and Status Standard]]：type、domain、priority、authoring / expression / learning status、coverage disposition 和 evidence maturity。
- `09` [[kernel/09 Wiki Link and Navigation Standard|Wiki Link and Navigation Standard]]：正文链接、结构导航、path、alias 和验证。
- `10` [[kernel/10 Writing and Formatting Standard|Writing and Formatting Standard]]：命名、公式、表格、图、rendering workflow，以及由 `Language Contract` 提供的 reader-facing language binding。
- `11` [[kernel/11 Expression Layer Standard|Expression Layer Standard]]：表达产物、coverage、readiness、evidence binding 与 migration audit；具体实现由 `Expression Layer Entry` 注册。
- `12` [[kernel/12 Quality Assurance Standard|Quality Assurance Standard]]：单篇、batch、Guidance / Coverage reconciliation、模块、source promotion、分层渲染和 Terminal Audit；扩展 QA dimensions、scans 与 gates 分别由 `Audit Dimension Registry`、`Registered Scan Registry` 和 `Routing And Gate Registry` 激活。

## Cross-domain Rule Registry

以下高危对象在全库只有唯一 canonical owner。修改这些对象只能修改 owner 文件；其它任何位置只允许通过 Wiki Link 或已注册 slot 引用，不允许复制内容（无论是否略有改写）。
...
| authoring / expression / learning 状态词表 | [[kernel/08 Metadata and Status/03 Status Axes|Status Axes]] + `Expression Status Axis` role |
...
| 维护轮预算封套 | [[kernel/00 Standards Control/02 Task Routing and Pre-execution|Task Routing and Pre-execution]] |
...
| Registered residual-content scan | 批次关闭：[[kernel/12 Quality Assurance/07 Audit Evidence Reuse and Invalidation#Batch-close Closed List|封闭清单]]第 6 项 + `Registered Scan Registry` residual-scan hook | 其它层引用封闭清单与已注册 hook，不另做全库扫描 |

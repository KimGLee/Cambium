...
A long task may be marked `complete` only through the closure frozen in its Task Contract. `completion_semantics: build` requires [[kernel/K12 Quality Assurance/03 Module and Coverage Review#Coverage Reconciliation Review|Coverage Reconciliation Review]], [[kernel/K12 Quality Assurance/04 Guidance and Source Review#Guidance Reconciliation Review|Guidance Reconciliation Review]], the `required-queue-completion` Gate, a current `corpus-plan-structure` result when Corpus Planning is configured, and the [[kernel/K12 Quality Assurance/15 Terminal Audit and Convergence#Terminal Audit|Terminal Audit]]. `completion_semantics: maintenance` instead requires the bounded predicate in K00/06 and, when persistent state applies, the `maintenance-completion` Gate defined by K13/12; it MUST NOT invoke Terminal Audit. A maintenance run that changed planning artifacts must still reconcile them before closing. The Queue may retain closed and authorized-cancelled history; either state-backed path requires zero remaining Required work rather than an empty Queue file.

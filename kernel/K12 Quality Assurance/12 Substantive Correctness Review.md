...
The review produces `substantive_review` evidence under the sole machine authority of [`substantive-review-contract.yaml`](substantive-review-contract.yaml). The unique producer route is capability `substantive-review-attestation-v1`; the `batch-review` Gate is its consumer. At the `pre-merge` due stage, a passing record discharges only the exact AuditPlan obligation it binds, as `substantive-review-evidence` in dimension `content_and_depth` against acceptance predicate `content-correctness`. First publication MUST accept that binding and the legal review round against current inputs, including locked pre-write verification and exact write read-back. A second object that only copies this already accepted binding is not required. The original review statement, findings, failed rounds, predecessor links, correction history and independent reviewer requirement remain intact.

...
Item 6 is the only member whose predicate is supplied entirely by the selected profile, so its configuration MUST be shown to be live and not merely present. The item reports the absence of the registered structures outside the accepted roots; that report is evidence only when the same configuration still recognises those structures where the profile declares they legitimately live. A run that reports no candidate and whose configuration also recognises nothing under its own accepted roots produced no reliable evidence and MUST fail rather than pass, and the passing summary MUST name the accepted-root object the matchers recognised. This is an evidence-production failure, not a content finding: item 6 still raises content findings only as candidates, and whether a live matcher describes the right structures remains the registering profile's judgment.

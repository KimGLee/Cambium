...
Sealing is the one operation that REMOVES bytes from a register, and that makes it the one operation appends must be excluded from. Every managed receipt append and every seal MUST take the same exclusive marker (`.cambium/tmp/receipt-append.free` renamed to `.held` and back — a rename, because a lock that cannot be released on a mount that refuses `unlink` is not a lock). Lock order is always the runtime writer lock, then this mutex. Comparing hashes on either side of the rewrite window narrows that window and cannot close it: a receipt appended inside it is dropped, and dropped invisibly, because the post-seal validation reads the evidence set the row is now missing from. An abandoned mutex is reclaimed only on proof that its recorded process no longer exists — never on a timeout, which cannot tell a slow writer from a dead one. Behind the mutex the rewrite still MUST carry across any tail it did not plan for: a seal may remove only the rows it sealed.

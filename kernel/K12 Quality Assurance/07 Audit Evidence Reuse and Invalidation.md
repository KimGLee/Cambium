...
Sealing MUST be a compare-and-swap. Every byte the plan was computed from — the three canonical state files and every file under `.cambium/receipts`, hot and cold — is compared again inside the writer lock, and any drift aborts before the first write; otherwise a receipt appended by a cooperating writer between planning and the lock would be dropped by a rewrite computed before it existed. Publication MUST be journalled: the `begin` row and a pending record land before the first segment byte and the `complete` row lands after the last hot rewrite, so a writer that dies mid-transaction leaves a state that fails every later run closed and that `seal_receipts.py --reconcile --apply` finishes deterministically rather than a state an operator must repair by hand.

...
- M-tier pages pass the canonical [[kernel/K12 Quality Assurance/01 Quality Dimensions and Single Note Review#M-tier Gate Checklist|M-tier checklist]] page by page. Its sole atomic projection is [`batch-review-obligation-registry.yaml`](batch-review-obligation-registry.yaml). The AuditPlan freezes emitting definitions, including conditional judgments; a conditional `not-applicable` requires a reason. Requirements already proved by planned evidence retain their rule identities and exact consumption selectors, not duplicate production obligations.
...
The registry owns the M/S evidence contract. Each `consumes` selector resolves to original planned evidence, without a second reviewer declaration. Wiki Link evidence covers missing and ambiguous links; batch rendering-boundary evidence proves the permitted level or exception, not compilation or layout. Profile-rendering absence requires the existing construct/contract closure; an unresolved selector or contract gap remains HOLD. Sampled-S evidence remains dimensionless, never a fabricated `AuditReceipt`.

M results reference their immutable AuditPlan, not copies of its definitions. Distinct facts retain statements, input/dependency fingerprints and independently correctable identities. The registered failure-applicability fact is decided once: genuine absence permits the accepted explicit-N/A result to cover the exact same-page conditional obligations, but it must attest to the page's explanation; a Receipt reason alone cannot replace that explanation. Otherwise, distinct answers bind the accepted condition. Withdrawal removes the common fact's coverage and dependent authority, not unrelated results. Consumers resolve these exact plan/registry relations, never an arbitrary record or blanket page pass. Publications remain independent: a later failure does not undo earlier valid results. Unaffected results remain reusable under K12/07.

...
- Terminal Proof completeness and zero-value condition validation (canonical definition in [[kernel/K12 Quality Assurance/15 Terminal Audit and Convergence#Terminal Audit|Terminal Audit]]) — implemented as `Tools/check_proof.py`; terminal mode reconciles the frozen Standards version and selected profile against the active K00/03 state and Progress Ledger, and may additionally reconcile Required gaps with the Coverage Ledger.

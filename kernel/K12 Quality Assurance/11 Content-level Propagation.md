...
When a note's mechanism sections (Definition, Mechanism, formulas, core conclusions) are substantively modified, the author MUST mark the direct downstream notes `needs_rereview` along the semantic dependency edges defined in [[kernel/K12 Quality Assurance/07 Audit Evidence Reuse and Invalidation#Dependency Invalidation|Dependency Invalidation]] (prerequisite, claim-evidence), and record this into the Coverage Ledger via the batch delta's `open_gaps_added` (type: rereview) — under concurrency the author does not write the Ledger directly ([[kernel/K02 Build Execution/05 Batch Execution|K02/05]] write partition). The marks flow into the maintenance run's candidate pool to be absorbed within budget; on-the-spot handling is not required. The same page is marked into the pool only once per maintenance run cycle. The re-review action is re-reading whether the downstream reasoning still holds, not re-running mechanical checks.

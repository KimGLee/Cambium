...
2. Establish or reconcile the `Global Map` before bulk deletion, migration, or corpus-wide expansion. One corpus cannot obey that order — the one that does not exist yet. The Map names existing canonical owners, so a corpus whose Profile Scope layer directories hold none has nothing to establish, and admitting its first task would otherwise require a plan of a corpus that is not there.

   Bootstrap ordering resolves it without relaxing any record contract. The selected profile still declares all three bindings, so [[kernel/K00 Standards Control/13 Runtime Admission and Recovery#Large-scale Pre-execution Gate|K00/13]] admits the task on a declared plan; the initial batch's manifest then carries both the canonical owners the Map will name and the three artifact paths themselves. Carrying the artifact paths is what gives the deferral teeth: it makes that batch's Corpus Planning close gate applicable by manifest, so `check_corpus_plan` runs there rather than not at all.

   What moves is when the plan is proved, never whether. The batch cannot close until the three artifacts exist, reflect the admitted scope, and reconcile against one another and the corpus; the Map still names only owners that exist when it is checked, and it is checked before that close. The branch is available only while no such owner exists — once one does, the ordinary order applies, and no batch after the first may use it.

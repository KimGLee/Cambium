...
This table is a control-plane quick view; the complete vocabularies are authoritative at each owner: for `task_state` see [[kernel/K02 Build Execution/01 Contract Time and Task State|K02/01]], for batch lifecycle and holds see [[kernel/K02 Build Execution/09 Required Queue|K02/09]], for the remaining kernel axes see [[kernel/K08 Metadata and Status/03 Status Axes|K08/03]], and expression status values are provided by the `Expression Status Axis` role.

...
|---|---|---|---|
| `runtime-card-synchronization` | Runtime Card completeness and source synchronization | Governance close runs `Tools/stamp_cards.py . --check` under the [[kernel/K00 Standards Control/03 Standards Governance#Revision Write-back Checklist|Revision Write-back Checklist]] | Routine tasks consume stamped Cards; profile loading cannot waive or recreate the gate |
...
| `standards-revalidation` | Deferred post-adoption changed-predicate gates at their declared boundary | K13/15 `check_queue.py --require-revalidation <boundary>` | The named boundary consumes exact Gate-ID receipts and records one current revalidation receipt; unrelated work does not run every gate |

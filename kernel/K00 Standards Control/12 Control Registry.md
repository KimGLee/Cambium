...
|---|---|---|---|
| `runtime-card-synchronization` | Runtime Card completeness and source synchronization | The [[kernel/K00 Standards Control/03 Standards Governance#Revision Write-back Checklist\|Revision Write-back Checklist]] `manual-attestation` signed at Governance close, with `Tools/stamp_cards.py . --check` as its input | Routine tasks consume stamped Cards; profile loading cannot waive or recreate the gate |

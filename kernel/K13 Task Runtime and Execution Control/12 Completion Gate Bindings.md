...
Progress freezes one completion path. Build uses
`required-queue-completion` before `completion-candidate` and again for
Terminal Proof. Maintenance uses
`maintenance-completion`, never task state
`completion-candidate` or Terminal Proof. It closes the candidate set, exact partition, Queue
...
revisions. Only the registered task-state transaction may consume it to mark
the task complete.
...
`corpus-plan-structure` receipt. The `terminal-proof` Gate consumes it from the declared
...
missing child blocks the Proof pass and therefore cannot be consumed by the
task-state completion transition.

...
Governance state and Standards-adoption receipts may exist before any task
runtime. Task state, Work Specs, deltas, and receipts are durable; `deltas/`
also contains
...
Queue, task ledger, or general documentation directory. Initialize task state
only when `.cambium/state/` is absent; `init_state.py` preserves a valid
governance-only parent namespace. Otherwise run
`check_queue.py --resume-status` before any write and follow its

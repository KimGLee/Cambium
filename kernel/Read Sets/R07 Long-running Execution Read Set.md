...
- A situation arises that no gate judges and the run may not decide alone — an unexempted quota overrun, a replan beyond the declared magnitude, a batch idle beyond the declared age, an irreversible operation, or any condition the selected Profile's `Escalation Policy` registers: read [[kernel/K13 Task Runtime and Execution Control/17 Escalation Policy|Escalation Policy]] for what firing one obliges, then suspend through K13/03 before starting further Required work.
- Startup discovery finds no runtime namespace and this task needs one: after `init_state.py` publishes it, read [[kernel/K13 Task Runtime and Execution Control/18 Initial Task Planning Transaction|Initial Task Planning Transaction]] for the sole writer that fills the empty Task Contract and Coverage from one confirmed plan, and for the compiler step that materializes the Queue afterwards. Never hand-edit those first values.

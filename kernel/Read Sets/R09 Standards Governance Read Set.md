...
- Before a later revision, freeze the exact canonical adopter Standards state bytes. For initial adoption, prove that state file is absent and freeze the upstream tag, commit, or archive checksum instead; an old profile does not exist and is not a prerequisite. In both branches, freeze affected modules, incoming links, and active task impact.

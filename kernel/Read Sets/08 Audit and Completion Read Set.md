...
- 图、表格、公式、图片、embed 或具体显示问题：读取 [[kernel/12 Quality Assurance/02 Rendering Verification|Rendering Verification]]。先审计 Level 0 / Level 1 确定性证据；只在存在记录的 visual exception trigger 时审计 UI、截图或录屏证据。
- Guidance 或 source promotion：读取 [[kernel/12 Quality Assurance/04 Guidance and Source Review|Guidance and Source Review]]；Expression Layer content：加载所选 profile 在 `Routing And Gate Registry` 登记的 specialized-review role。
- 目录迁移：读取 [[kernel/Read Sets/06 Migration and Refactor Read Set|Migration and Refactor]] 的 Gate modules。

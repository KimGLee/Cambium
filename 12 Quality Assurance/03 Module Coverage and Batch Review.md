...
- Previous: [[Knowledge Base Standards/12 Quality Assurance/02 Rendering Verification|Rendering Verification]].
- Next: [[Knowledge Base Standards/12 Quality Assurance/04 Guidance Source and Interview Review|Guidance Source and Interview Review]].
...
- Agent/Harness 主线依赖的 ML、DL、LLM、Retrieval 和系统基础是否仍然完整。
- 是否存在重复 canonical notes。
- 是否存在 orphan notes。
- Roadmap、Cheat Sheet 和 Interview Preparation 是否同步。
...
Module Review 先消费已关闭 batches 的有效 AuditReceipts，再审查跨 batch 才能判断的 owner、dependency、coverage 和 navigation invariants。没有相关变化的局部 mechanism 不应被逐页重审；receipt 缺失、失效或抽样暴露系统问题时，按 [[Knowledge Base Standards/12 Quality Assurance/07 Audit Evidence Reuse and Invalidation|Audit Evidence Reuse and Invalidation]] 扩大范围。
...
- Roadmap checkbox、文件存在、Wiki link 可解析和 `Related` 引用没有被当作 authoring completion。
...
并发执行时，批次正文只能链接到已合并内容或本批清单内的页面；指向在途批次页面的链接留待双方批次合并后补充：作者把缺链记入本批 delta 的 `open_gaps_added`（type: link），由维护轮按预算消化；补链位置遵循 [[Knowledge Base Standards/12 Quality Assurance/01 Quality Dimensions and Single Note Review|12/01]]（Related 不是唯一引用位置）。
...
- S 档页面按抽样复核：默认抽取 `max(2, 20%)` 的本批 S 档页面（不足 2 个则全查）；抽样发现问题时按 [[Knowledge Base Standards/12 Quality Assurance/07 Audit Evidence Reuse and Invalidation|Audit Evidence Reuse and Invalidation]] 扩大范围。
- M 档页面在 batch gate 内逐页过对应 Card 的 Gate 清单。
- L 档页面保留独立 note gate，按 [[Knowledge Base Standards/12 Quality Assurance/01 Quality Dimensions and Single Note Review|12/01]] 全量执行，不并入本节。
...
- Interview migration 按优先级完成或有明确 disposition。
...
- guidance 对账见 [[Knowledge Base Standards/12 Quality Assurance/04 Guidance Source and Interview Review|12/04]]（增量）。

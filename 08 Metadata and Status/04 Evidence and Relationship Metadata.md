...
- Previous: [[Knowledge Base Standards/08 Metadata and Status/03 Status Axes|Status Axes]].
- Next: [[Knowledge Base Standards/08 Metadata and Status/05 Review Source and Migration Metadata|Review Source and Migration Metadata]].

## Evidence Maturity

`authoring_status` 表示页面写作与审阅进度；`evidence_maturity` 表示时效性或经验性结论的证据强度。两者不能互相替代。

旧页面中的 `status` 字段在迁移期间视为 `authoring_status` 的兼容别名，不再用于表达 Interview 或学习状态。
...
- `superseded`：结论已被更新证据或更准确的模型取代。

稳定数学定义通常不需要 `evidence_maturity`。前沿 Agent/Harness 模式、行业经验、benchmark 结论和 Research Synthesis 应填写。

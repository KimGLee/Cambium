...
- Previous: [[Knowledge Base Standards/10 Writing and Formatting/04 Rendering and Formatting Review|Rendering and Formatting Review]].
...
`Knowledge Base Standards/` 语料自身（标准与管理页面）豁免“英文标题必须加中文注释”的要求，理由：控制面文件以稳定英文标题保证 heading anchor 稳定性与工具兼容。本豁免仅适用于标准语料，不适用于知识正文。

## File And Folder Names（文件与文件夹名称）

- 文件夹和文件名只使用英文正式名称或行业通用缩写，不追加中文注释、中文翻译或中英双语后缀（验收项见 Acceptance And Audit）。
...
Standards v1.5 从本规则开始采用中文优先的可执行验收，不要求无差别立即重写全部历史页面。
...
- [[Knowledge Base Standards/10 Writing and Formatting Standard|Writing and Formatting Standard]]

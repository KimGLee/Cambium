...
def _projection_string(value, label, *, nullable=False):
    if value is None and nullable:
        return None
    return _support.nonempty_string(value, label)
...
def validate_contract(document):
    ...
    if document.get("schema_version") != 1:
        raise ValueError("substantive-review schema_version must be 1")
    ...
    required_fields = {
        ...
        "page_sha256", "sources_sha256", "semantic_content_fingerprint",
        ...
        "reviewer_context_id", "reviewer_role", "round",
        "round_1_receipt_id", "verdict", "findings",
    }
    required_finding_fields = {
        "finding_id", "severity", "statement", "status",
        "round_1_finding_id",
    }
    if set(fields) != required_fields:
        raise ValueError(
            "substantive-review instance fields do not match K12/12")
    if set(finding_fields) != required_finding_fields:
        raise ValueError(
            "substantive-review finding fields do not match K12/12")
    results = _support.closed_string_list(
        document.get("result_values"), "result_values")
    if set(results) != {"pass", "fail"}:
        raise ValueError("substantive-review results must be pass/fail")
    ...
    for index, row in enumerate(mappings):
        ...
        verdict = _support.nonempty_string(
    ...
    if verdict_results != {
            "passed": "pass", "changes-required": "fail",
            "escalated": "fail"}:
        raise ValueError("substantive-review verdict mapping is invalid")
    ...
    if set(severities) != {"critical", "major", "minor"}:
        raise ValueError("substantive-review severity namespace is invalid")
    if set(blocking) != {"critical", "major"}:
        raise ValueError("substantive-review blocking severity set is invalid")
    if set(statuses) != {"open", "closed", "recorded"}:
        raise ValueError("substantive-review finding statuses are invalid")
    return {
...
def validate_review_receipt(record, contract=None):
    ...
    if round_number not in (1, 2):
        raise ValueError("substantive-review round must be 1 or 2")
...
def validate_review_pair(first, second, contract=None):
    ...
    binding_fields = (
        "plan_id", "audit_plan_sha256", "obligation_id", "task_id",
        "batch_id", "opening_transition_receipt", "standards_version",
        "active_standards_sha256", "selected_profile_manifest",
        "profile_snapshot_sha256", "profile_contract_fingerprint", "target",
        "page_sha256", "sources_sha256", "semantic_content_fingerprint",
        "artifact_fingerprint", "dependency_fingerprint",
        "contract_fingerprint", "fingerprint_binding",
        "acceptance_predicate", "authoring_context_id",
    ...
    for row in second["findings"]:
        if row["severity"] != first_by_id[row["round_1_finding_id"]][
                "severity"]:
            raise ValueError("round 2 changes a finding severity")
    return second
...
__all__ = [
    "FINDING_FIELDS", "OBLIGATION_PROJECTION", "REVIEW_RECEIPT_FIELDS",
    "ROUND_CAP",
    "SUBSTANTIVE_REVIEW_CONTRACT_PATH", "load_contract",
    "validate_contract", "validate_review_pair", "validate_review_receipt",
]

...
import deterministic_rendering_contract
import kblib
...
def _producer_route(row, label):
    ...
    route = _nonempty(row.get(route_field), "%s.%s" % (label, route_field))
    return (
...
def validate_changed_scope_registry(document):
...
def _spec(_registered_dimensions=None, **values):
...
def _substantive_specs(root, snapshots):
...
def _batch_review_specs(root, snapshots):
...
def _changed_scope_specs(root, snapshots):
    ...
    deterministic_rendering_contract.validate_registry_projection(
        registry,
        deterministic_rendering_contract.load_contract(root), root=root)
    result = []
...
def _batch_close_specs(root, snapshots):
...
def _reject_unadmitted_rendering_specs(specs, root, snapshots=None):
    ...
    rendering = rendering_verification_contract.validate_contract(
        rendering_verification_contract.load_contract(root, snapshots))
    admitted_owner_ids.add(
        rendering["obligation_projection"]["owner_rule_id"])
...
def base_obligation_specs(root=None, snapshots=None):
    ...
    root = _root(root)
    specs = (
        _substantive_specs(root, snapshots) +
        _batch_review_specs(root, snapshots) +
        _changed_scope_specs(root, snapshots) +
        _batch_close_specs(root, snapshots)
    )
...
def s_tier_sample_count(population_count, root=None, snapshots=None):
    """Delegate the registered ceiling arithmetic to the K12/14 loader."""
    registry = batch_review_obligation_contract.load_registry(root, snapshots)
    return batch_review_obligation_contract.s_sample_count(
        population_count, registry)
...
def resolve_obligation_definition(spec, target, trigger=None, dimension=None,
    ...
    target = _nonempty(target, "target")
    ...
    if spec["dimension_binding"] == "profile-registration":
        resolved_dimension = _nonempty(dimension, "dimension")
...
__all__ = [
    "BATCH_CLOSE_REGISTRY_PATH", "BATCH_REVIEW_REGISTRY_PATH",
    "CHANGED_SCOPE_REGISTRY_PATH", "SUBSTANTIVE_REGISTRY_PATH",
    "base_obligation_specs", "compose_profile_extensions",
    "load_changed_scope_registry", "obligation_spec_for_rule",
    "resolve_obligation_definition", "s_tier_sample_count",

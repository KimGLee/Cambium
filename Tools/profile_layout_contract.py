...
MINIMAL_TEMPLATE_PROFILE_ID = "_template-minimal"
EXAMPLES_PROFILE_ID = "examples"
TEMPLATE_PROFILE_IDS = frozenset((
    TEMPLATE_PROFILE_ID,
    MINIMAL_TEMPLATE_PROFILE_ID,
))
...
def profiles_directory(root):
    """Return the filesystem path of the one shipped Profile namespace."""
    return Path(root) / PROFILES_DIRECTORY
...
def profile_manifest_relative(profile_id):
    """Return the canonical flat manifest path for one namespace member."""
    return "%s/%s" % (profile_relative(profile_id), PROFILE_MANIFEST_NAME)

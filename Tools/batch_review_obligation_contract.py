...
def _acceptance_contract(value, label):
    ...
    return {
        "contract_id": _nonempty(
            value.get("contract_id"), label + ".contract_id"),
        "text": _nonempty(value.get("text"), label + ".text"),
    }
...
def _trigger_mappings(value, label, plan_values):
    ...
    for index, row in enumerate(value):
        ...
        trigger = _nonempty(row.get("trigger"), row_label + ".trigger")
        partition = _nonempty(
...
def _producer_field_specs(value):
    ...
    for index, row in enumerate(value):
        ...
        field = _nonempty(row.get("field"), label + ".field")
...
def _producer_contract(value):
    ...
    for index, row in enumerate(variants):
        ...
        tier = _nonempty(row.get("tier"), row_label + ".tier")
    ...
    for index, row in enumerate(mappings):
        ...
        verdict = _nonempty(row.get("verdict"), row_label + ".verdict")
        result = _nonempty(row.get("result"), row_label + ".result")
...
def validate_registry(document):
    ...
    if document.get("schema_version") != 1:
        raise ValueError("batch-review registry schema_version must be 1")
    ...
    for field in (
            ...
            "fingerprint_binding"):
        _nonempty(projection.get(field), "audit_plan_projection.%s" % field)
    ...
    for trigger, meaning in meanings.items():
        _nonempty(meaning, "trigger_meanings.%s" % trigger)
    ...
    for index, row in enumerate(groups):
        ...
        group_id = _nonempty(row.get("group_id"), label + ".group_id")
        ...
        _nonempty(row.get("source_text"), label + ".source_text")
    ...
    for index, row in enumerate(items):
        ...
        item_id = _nonempty(row.get("item_id"), label + ".item_id")
        rule_id = _nonempty(row.get("rule_id"), label + ".rule_id")
        group_id = _nonempty(
...
def validate_applicability_disposition(spec, disposition, reason,
    ...
    if disposition == "applicable":
        ...
        _nonempty(reason, "not-applicable reason")
    return {
...
def resolve_consumed_evidence(plan, plan_sha256, spec, target, catalog,
    ...
    current-catalog view and returns the exact sorted evidence rows; callers
    cannot substitute an unrelated passing receipt.  An unresolved selector
    is an explicit HOLD, not a permissive fallback.
    """
    registry = registry or _SHIPPED_REGISTRY
    ...
    target = _nonempty(target, "consumption target")
    ...
    for obligation in plan.get("obligations") or ():
        if (obligation.get("owner_kind") == selector["owner_kind"] and
                ...
                selector["consumer_gate_id"]):
            expected_obligations.append(obligation)
    expected_obligations.sort(key=lambda row: row["obligation_id"])
    cardinality = selector["match_cardinality"]
    if cardinality == "exactly-one" and len(expected_obligations) != 1:
        raise ValueError(
            "M consumption selector %s requires exactly one AuditPlan "
            "obligation, found %d" %
            (selector["selector_id"], len(expected_obligations)))
    if cardinality == "one-or-more-all-matching-required" and not \
            expected_obligations:
        raise ValueError(
...
def partition_for_trigger(spec, trigger):
    """Resolve an M spec's planning trigger without changing obligations."""
    trigger = _nonempty(trigger, "trigger")
    mappings = spec.get("trigger_partition_mappings") or ()
    matches = [row["partition"] for row in mappings
               if row["trigger"] == trigger]
    if len(matches) != 1:
        raise ValueError("rule %s has no unique partition for trigger %s" %
                         (spec.get("rule_id"), trigger))
    return matches[0]


def _plan_projection_errors(obligation, spec):
    expected = {
...
def validate_plan_base_closure(plan, manifest, tiers, registry=None):
    ...
    for obligation in plan["obligations"]:
        ...
        errors = _plan_projection_errors(obligation, spec)
        if errors:
            raise ValueError(
...
def select_s_targets(population, *, task_id, batch_id,
    ...
    opening_transition_receipt = _nonempty(
...
def contract_fingerprint(spec, bindings, registry=None):
    ...
    material = {
        ...
        "standards_version": bindings.get("standards_version"),
...
def validate_producer_receipt(record, registry=None):
    ...
    if record.get("schema_version") != 1:
        raise ValueError("batch-page review schema_version must be 1")
...
__all__ = [
    "BATCH_PAGE_REVIEW_RECORD_FIELDS",
    "BATCH_REVIEW_OBLIGATION_REGISTRY_PATH", "M_ATOMIC_ITEM_IDS",
    "M_ATOMIC_RULE_IDS", "PRODUCER_TOOL", "PRODUCER_TOOL_VERSION",
    "S_SAMPLING_RULE_ID", "S_SELECTION_ALGORITHM_ID",
    "base_obligation_specs", "contract_fingerprint",
    "dependency_fingerprint", "load_registry", "obligation_spec_for_rule",
    "partition_for_trigger", "registry_sha256", "s_sample_count",
    "select_s_targets", "validate_plan_base_closure",
    "resolve_consumed_evidence", "validate_applicability_disposition",
    "validate_page_fingerprint_binding", "validate_producer_receipt",
    "validate_receipt_consumption", "validate_registry",
]

...
from queue_runtime.task_contract import live_read_set_load_findings
...
def coverage_routing_state(coverage, queue, targets=None):
    """Check K12/05's three Coverage/routing observations in one scope."""
    ...
    for path in selected:
        ...
        if record is None:
            diagnostics.append(_diagnostic(
                ...
                "missing"))
            continue
        if record.get("authoring_status") == "unassessed":
            counts["unassessed"] += 1
            diagnostics.append(_diagnostic(
                "coverage-unassessed", target, "authoring_status",
...
__all__ = [
    "CHECKS_BY_RULE_ID", "COVERAGE_CHECK_ID", "COVERAGE_RULE_ID",
    "GUIDANCE_CHECK_ID", "GUIDANCE_RULE_ID", "TASK_CONTRACT_CHECK_ID",
    "TASK_CONTRACT_RULE_ID", "coverage_routing_state",
    "frozen_task_contract_references", "guidance_state_zero_counts",

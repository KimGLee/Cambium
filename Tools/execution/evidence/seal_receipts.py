...
that later goes missing or changes fails every consistency run closed.  The
planner never treats a filename, a batch-looking string, or an unknown
producer as authority to seal.  It first excludes every body that the typed
Receipt-reference graph says a current runtime object must keep hot, then
admits only a registered seal class.  Global runtime histories, Standards
adoptions, contract and operational amendments, this tool's own receipts,
open-batch evidence, and current Coverage evidence therefore remain hot.  A
producer batch may be closed while its review/content-change receipt is still
live owner evidence; only a later owner transition that replaces the pointer
makes that row cold.
...
Registered seal classes:
  close-graph             members reached through a terminal Queue item's
                          typed close-bundle graph
  terminal-batch          recognized close/page-review/audit/delta producer
                          bodies for a transition-anchored closed batch
  page-contract-snapshot  recognized advisory page-contract evidence

Runtime-object identity supplies explicit permanent-hot exclusions.  Typed
graph reachability supplies current body requirements.  Producer shape plus
terminal state supplies positive seal classification.  Unknown or ambiguous
rows remain hot by default.
"""
...
from Tools.execution.task_runtime.queue_runtime.canon import (
    SEAL_TOOL as TOOL,
    SEAL_TOOL_VERSION as TOOL_VERSION,
)
...
RECEIPT_TYPE_ID = "receipt-seal-receipt-v1"
...
def current_receipt_errors(record, *, root=None):
    errors = receipt_type_contract.base_receipt_errors(
        ...
        tool=TOOL, tool_version=TOOL_VERSION, checks="receipt_seal")
    if isinstance(record, dict):
        for field in ("manifest_rows_sha256", "index_rows_sha256",
                      "sealed_segments", "sealed_records"):
            if field not in record:
                errors.append("seal Receipt misses %s" % field)
    return errors

SEAL_CLASS_CLOSE_GRAPH = "close-graph"
SEAL_CLASS_TERMINAL_BATCH = "terminal-batch"
SEAL_CLASS_PAGE_SNAPSHOT = "page-contract-snapshot"
REGISTERED_SEAL_CLASSES = frozenset((
    SEAL_CLASS_CLOSE_GRAPH,
    SEAL_CLASS_TERMINAL_BATCH,
    SEAL_CLASS_PAGE_SNAPSHOT,
))

# Physical identity is read from the runtime-path owner, not re-spelled as a
# basename list.  These append-only global histories have no cold consumer in
# the implemented protocol and therefore remain hot even when no current
# Ledger field names a particular row.
PERMANENT_HOT_RUNTIME_OBJECT_IDS = frozenset((
    "queue-transition-receipts",
    "task-transition-receipts",
    "standards-adoption-receipts",
    "contract-amendment-receipts",
    "amendment-receipts",
    "task-plan-receipts",
    "seal-receipts",
))
_RUNTIME_OBJECT_ID_BY_PATH = {
    spec.path: object_id
    for object_id, spec in runtime_paths.RUNTIME_OBJECTS.items()
}

# Only these typed Receipt bodies may become cold merely because their batch
# reached the terminal boundary.  Unknown producers and arbitrary rows that
...
def _closed_anchored_batches(result):
    ...
    for item_id, item in (result.get("items_by_id") or {}).items():
        if item.get("state") != "closed":
            continue
        transitions = receipt_reference_contract.edge_reference_ids(
            item, receipt_reference_contract.SOURCE_ITEM,
            "queue-item.transition", recursive=False)
        anchored = any(
            (catalog.get(rid) or (None, {}))[1].get("after_state") == "closed"
            for rid in transitions)
        if anchored:
            closed.add(item_id)
    return closed


def _row_batch(receipt, closed):
    batch = receipt.get("batch_id")
    if isinstance(batch, str) and batch in closed:
        return batch
    target = receipt.get("target")
    if isinstance(target, str) and target in closed:
        return target
    return None


def _registered_seal_class(relative, receipt_id, receipt, closed,
                           close_graph_ids, type_registry):
    """Classify one row through the sole implemented seal-class contract.

    A source filename is never sufficient evidence.  Exact registered runtime
    identity can refuse sealing; positive admission comes from typed graph
    reachability, a recognized Receipt-body producer shape plus a terminal
    batch, or the recognized advisory page-contract producer.  Anything else
    is intentionally unclassified and remains hot.
    """
    object_id = _RUNTIME_OBJECT_ID_BY_PATH.get(relative)
    if object_id in PERMANENT_HOT_RUNTIME_OBJECT_IDS:
        return None
    if receipt_id in close_graph_ids:
        return SEAL_CLASS_CLOSE_GRAPH
    try:
        source_kind = receipt_type_contract.reference_source_kind(
            receipt, registry=type_registry)
    except receipt_type_contract.ReceiptTypeContractError:
        # A row without a current typed producer has no seal class.  The
        # planner must leave it hot; parsing it as an older producer format
        # would reintroduce a retired compatibility path.
        return None
    if (source_kind in TERMINAL_BATCH_SOURCE_KINDS and
            _row_batch(receipt, closed) is not None):
        return SEAL_CLASS_TERMINAL_BATCH
    if (receipt.get("tool") == "check_page_contract" and
            receipt.get("gate_id") == "page-contract" and
            isinstance(receipt.get("check"), str) and
            receipt["check"].startswith("page-contract")):
        return SEAL_CLASS_PAGE_SNAPSHOT
    return None
...
def _closed_bundle_trios(result, closed):
    ...
    for item_id, item in (result.get("items_by_id") or {}).items():
        if item_id not in closed:
            continue
        trio_ids.update(receipt_reference_contract.reference_ids(
            item, receipt_reference_contract.SOURCE_ITEM,
            recursive=False,
            closure=receipt_reference_contract.CLOSE_BUNDLE_CLOSURE))
    return trio_ids


def _closed_bundle_graph(result, closed):
    """Return the complete typed close graph for terminally closed batches."""
    catalog = result.get("receipt_catalog") or {}
    members = _closed_bundle_trios(result, closed)
    for item_id, item in (result.get("items_by_id") or {}).items():
        if item_id not in closed:
            continue
        close_ids = receipt_reference_contract.edge_reference_ids(
            item, receipt_reference_contract.SOURCE_ITEM,
            "queue-item.close-gate", recursive=False)
        close_id = next(iter(close_ids), None)
        entry = catalog.get(close_id)
        close = entry[1] if isinstance(entry, tuple) and len(entry) == 2 \
            else None
        members.update(_hot_close_replay_receipts(catalog, close))
    return members
...
def _hot_close_replay_receipts(catalog, close):
    ...
    def resolve_body(receipt_id):
        entry = catalog.get(receipt_id)
        return (entry[1] if isinstance(entry, tuple) and len(entry) == 2 and
                isinstance(entry[1], dict) else None)

    return receipt_reference_contract.walk_receipt_closure(
        close,
        receipt_reference_contract.SOURCE_CLOSE,
        resolve_body,
        receipt_reference_contract.CLOSE_BUNDLE_CLOSURE,
        lambda body: receipt_type_contract.reference_source_kind(
            body, registry=catalog._type_registry),
        minimum_materialization=
            receipt_reference_contract.MATERIALIZATION_BODY_REQUIRED,
    )
...
def _hot_closed_bundle_receipts(result, closed, hot_references):
    ...
    for item_id, item in (result.get("items_by_id") or {}).items():
        if item_id not in closed:
            continue
        trio = receipt_reference_contract.reference_ids(
            item, receipt_reference_contract.SOURCE_ITEM,
            recursive=False,
            closure=receipt_reference_contract.CLOSE_BUNDLE_CLOSURE)
        close_ids = receipt_reference_contract.edge_reference_ids(
...
def _transition_bound_aggregates(catalog, transition_id):
    ...
    if not isinstance(transition, dict):
        return ()
    return receipt_reference_contract.reference_ids(
        transition, receipt_reference_contract.SOURCE_TRANSITION,
        recursive=False,
        closure=receipt_reference_contract.REVALIDATION_CLOSURE,
        minimum_materialization=
            receipt_reference_contract.MATERIALIZATION_BODY_REQUIRED,
        keep_hot=True,
    )
...
def _hot_reference_ids(result):
    ...
    property_evidence = receipt_reference_contract.reference_ids(
        coverage, receipt_reference_contract.SOURCE_COVERAGE,
        keep_hot=True)
    keep.update(property_evidence)
    # A current property owner keeps the bodies it directly consumes hot:
    # today that includes page-review attestations and Delta opening
    # transitions.  Do not recursively strengthen their own historical
    # identity edges; doing so would shrink the existing seal boundary.
    for receipt_id in property_evidence:
        receipt = resolve_body(receipt_id)
        source_kind = receipt_type_contract.reference_source_kind(
            receipt, registry=type_registry)
        if source_kind is not None:
            keep.update(receipt_reference_contract.reference_ids(
                receipt, source_kind, recursive=True,
                minimum_materialization=
                    receipt_reference_contract.MATERIALIZATION_BODY_REQUIRED))

    for _item_id, item in (result.get("items_by_id") or {}).items():
        state = item.get("state")
        references = tuple(receipt_reference_contract.iter_receipt_references(
            item, receipt_reference_contract.SOURCE_ITEM, recursive=True))
        for reference in references:
            if reference.spec.keep_hot:
                keep.add(reference.receipt_id)
            elif (state != "closed" and reference.spec.closure ==
                  receipt_reference_contract.CLOSE_BUNDLE_CLOSURE):
                keep.add(reference.receipt_id)
        transition_ids = {
            reference.receipt_id for reference in references
            if reference.spec.edge_id == "queue-item.transition"
        }
        for transition_id in transition_ids:
            keep.update(_transition_bound_aggregates(
                catalog, transition_id))

    progress = result.get("progress") or {}
    keep.update(receipt_reference_contract.reference_ids(
        progress, receipt_reference_contract.SOURCE_PROGRESS,
        keep_hot=True))
    active_standards = result.get("_active_standards_authorized_view") or {}
    keep.update(receipt_reference_contract.reference_ids(
        active_standards,
        receipt_reference_contract.SOURCE_STANDARDS_STATE,
        keep_hot=True))

    # The CLI refuses active/interrupted locks before planning.  Keeping their
    # declared phase edges here also makes direct planner callers conservative.
    for lock in result.get("_writer_locks") or []:
        ...
        if isinstance(operation, dict):
            keep.update(receipt_reference_contract.reference_ids(
                operation,
                receipt_reference_contract.SOURCE_WRITER_OPERATION,
                keep_hot=True))
    return keep
...
def plan_seal(root, result):
    ...
    if type_registry is None:
        raise ValueError("current Receipt type registry is unavailable")
    ...
    close_graph_ids = _closed_bundle_graph(result, closed)
    ...
    for receipt_id, (relative, receipt) in catalog.items():
        ...
        if receipt_id in referenced_hot:
            continue
        seal_class = _registered_seal_class(
            relative, receipt_id, receipt, closed, close_graph_ids,
            type_registry)
        if seal_class in REGISTERED_SEAL_CLASSES:
            by_file.setdefault(relative, []).append((receipt_id, receipt))
    return by_file
...
def _projection(receipt_id, segment, line_number, raw_line, receipt):
    ...
    for field in receipt_reference_contract.RECEIPT_COLD_PROJECTION_FIELDS:
        if field in receipt:
...
def _pending_record(root, result, receipts_path, manifest_rows, index_rows,
    ...
    seal_receipt = kblib.make_receipt(
        ...
        1, receipt_type_id=RECEIPT_TYPE_ID, root=root)
    ...
    seal_receipt.update({
        ...
        "manifest_rows_sha256":
            queue_runtime.receipt_group_sha256(manifest_lines),
        "index_rows_sha256":
            queue_runtime.receipt_group_sha256(index_lines),
...
def _publish(root, pending):
    ...
    if not already:
        outcome, error, _ = kblib.write_receipts_observed(
            receipt_full, _JSON_REPORTER.record([pending["receipt"]]))
...
def _cold_errors(root):
    errors = []
    catalog = queue_runtime.receipt_catalog(root, errors)
    store = queue_runtime.cold_receipt_store(root, errors, catalog)
    return store, errors
...
def _load_pending(root, entry, errors):
    ...
    if (queue_runtime.receipt_group_sha256(
            pending.get("manifest_lines") or []) !=
            receipt.get("manifest_rows_sha256") or
            queue_runtime.receipt_group_sha256(
                pending.get("index_lines") or []) !=
...
def _run(args):
    ...
    try:
        ...
        print("[FAIL] Receipt reference graph: %s" % exc)
        return 1
    ...
    after = runtime_validation.validate_runtime(root)
    if after["errors"]:

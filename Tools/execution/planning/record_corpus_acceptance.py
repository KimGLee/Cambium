...
RECEIPT_TYPE_ID = "corpus-plan-semantic-acceptance-receipt-v1"
...
def current_receipt_errors(record, *, root=None):
    ...
    if isinstance(record, dict) and record.get("gate_id") != GATE_ID:
        errors.append("gate_id must identify corpus-plan-semantic-acceptance")
    return errors
DEFAULT_RECEIPTS = runtime_paths.CORPUS_PLAN_ACCEPTANCE_RECEIPT_PATH


def _output(payload):
    reporting.write_canonical_json(payload)
...
def _make_receipts(result, plan, plan_path, plan_sha, snapshot):
    ...
    semantic = kblib.make_receipt(
        ...
        2, receipt_type_id=RECEIPT_TYPE_ID, root=result.get("root"),
    )

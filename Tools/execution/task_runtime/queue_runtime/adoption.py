...
from Tools.execution.task_runtime.queue_runtime.gate_registry import (
...
from Tools.execution.task_runtime.queue_runtime.item_history import invalidated_receipt_consumers
...
from Tools.execution.task_runtime.queue_runtime.profile_view import profile_load_evidence
from Tools.execution.task_runtime.queue_runtime.receipts import require_receipt
from Tools.execution.task_runtime.queue_runtime.repofs import repository_path_refusal
from Tools.execution.task_runtime.queue_runtime.task_contract import (
...
STANDARDS_ADOPTION_PLAN_FIELDS = frozenset((
    ...
    "upstream_revision_id_before", "upstream_revision_id_after",
    ...
    # The current contract names the upstream source explicitly; absent is
    # not an answer and no retired producer shape is inferred.
    "upstream_source_ref",
    # Adopter-owned Standards state is a separate runtime object.
...
def standards_adoption_plan_errors(
    ...
    Current admission validates live before-images. Historical replay validates
    the same current contract without reinterpreting immutable after-images as
    live authority.
    """
    profile_contract_required = True
    ...
    if isinstance(plan, dict) and \
            not (set(("upstream_source_ref", "upstream_revision_id_after")) -
                 set(plan)):
        source = plan.get("upstream_source_ref")
        revision = plan.get("upstream_revision_id_after")
        ...
        if not upstream_identity.is_full_commit_sha(revision):
            errors.append(
                "Standards adoption plan upstream_revision_id_after must be "
                "one full Git commit SHA")
    ...
    if validate_live_inputs and root is not None:
        ...
        if governance != expected_governance:
            ...
            try:
                ...
                errors.append("governance revision is unsafe or unreadable: %s" %
                              exc)
        current_state, current_view, state_errors = standards_state.snapshot(root)
        errors.extend("active Standards state: %s" % error
                      for error in state_errors)
        if current_view is not None:
            if current_view["active_standards_sha256"] != plan.get(
                    "standards_state_sha256_before"):
                errors.append("standards_state_sha256_before is stale")
            if current_state.get("upstream_revision_id") != plan.get(
                    "upstream_revision_id_before"):
                errors.append(
                    "active Standards state does not match plan before version")
            if current_state.get("selected_profile_manifest") != plan.get(
                    "selected_profile_manifest_before"):
                errors.append(
                    "active Standards state does not match plan before Profile")
    ...
    for index, predicate in enumerate(predicates):
        ...
        elif validate_live_inputs and root is not None:
            path_error_message = repository_path_refusal(
                root, owner, must_exist=True)
            if path_error_message:
    ...
    for index, boundary in enumerate(boundaries):
        ...
        if not validate_live_inputs:
            # A current-contract historical plan froze its owner closure in
            # required_gate_ids. Validate that frozen closure without
            # re-projecting it through a later live capability table. Retired
            # producer contracts never reach this validator.
        ...
        if boundary.get("target_kind") == "profile-load":
    ...
    if profile_contract_required:
        if validate_live_inputs:
            ...
            # The current owner closure is already frozen in each boundary;
            # Current-contract history does not reinterpret semantic leaves through
    ...
    reason_codes = frozenset((
        "predicate-changed", "profile-binding-changed",
    ...
    # direct-target plus evidence-scope union derived above. Current-contract
    # history remains immutable and is never re-admitted as live authority.
    if validate_live_inputs and queue is not None:
        terminal_states = {
    ...
    # Current admission also checks the live Queue reachability that cannot be
    # reconstructed from a later snapshot. Current-contract history retains
    # its frozen plan bytes and is checked separately without treating today's
    # Queue as the original before-image. Retired contracts never enter this
    # path.
    if validate_live_inputs and queue is not None and boundaries:
    ...
    # legitimate.
    #
    # The current admission path checks the live Read Set closure. A
    # current-contract historical plan keeps its frozen load declaration but
    # is not reinterpreted through a later repository snapshot. Retired plan
    # formats are rejected before this distinction is reached.
    if validate_live_inputs and root is not None:
    ...
    if predicate_set:
        blocking_predicate_set = (set(
            ...
            if validate_live_inputs else
            set(covered_predicates))
    ...
    if validate_live_inputs and root is not None:
        ...
        #
        # This reachability check depends on the live Queue and therefore
        # belongs to admission. Current-contract history validates its frozen
        # plan without substituting a later Queue snapshot; retired contracts
        # are not accepted as historical input.
        if queue is not None:
    ...
    if validate_live_inputs and root is not None and queue is not None and \
            catalog is not None:
    ...
    if validate_live_inputs and progress is not None and isinstance(
...
def standards_adoption_errors(
    ...
    errors = []
    previous = None
    accounted = accounted_upstream_revision_ids(progress, queue)
    for index, record in enumerate(records):
        ...
        for field in (
                "profile_contract_fingerprint_after",
                "profile_load_inputs_sha256_after", "upstream_source_ref",
            ...
            if field not in record:
                errors.append(
                    "%s misses %s required by %s" %
                    (label, field, STANDARDS_ADOPTION_TOOL_VERSION))
        errors.extend("%s %s" % (label, error)
                      for error in standards_adoption_plan_errors(
                          root, plan, catalog=catalog, queue=queue,
                          progress=progress, validate_live_inputs=False))
        changed_ids = sorted(
        ...
        record_plan_fields = {
            ...
            "upstream_revision_id_before": "upstream_revision_id_before",
            "upstream_revision_id_after": "upstream_revision_id_after",
            "selected_profile_manifest_before":
                "selected_profile_manifest_before",
        ...
        # Historical current-contract evidence remains bound to the exact
        # upstream identity that produced it.
        errors.extend(historical_receipt_identity_errors(
            receipt, receipt_id, "%s commit" % label, accounted,
            revision_field="upstream_revision_id_after"))
        if receipt is not None:
            ...
            if not isinstance(immediate_ids, list) or len(immediate_ids) != 1:
                ...
                # Historical current-contract gate already consumed by the
                ...
                require_receipt(
                    ...
                    errors, expected={
                        ...
                        "upstream_revision_id":
                            record.get("upstream_revision_id_after"),
                        "selected_profile_manifest":
                            record.get("selected_profile_manifest_after"),
                    })
        if previous is not None:
            if (record.get("upstream_revision_id_before") !=
                    previous.get("upstream_revision_id_after")):

...
from Tools.execution.task_runtime.queue_runtime.policy_exceptions import policy_exception_errors
...
CONTRACT_FIELDS = frozenset((
    ...
    "scope_version",
    "concurrency_cap",
    "upstream_revision_id", "selected_profile_manifest", "selected_route_ids",
...
def _standards_adoption_shape_errors(progress):
    ...
    for index, record in enumerate(records):
        ...
        for field in (
                ...
                "task_state_before", "upstream_revision_id_before",
                "contract_version_before", "contract_version_after",
                "upstream_revision_id_after", "selected_profile_manifest_before",
...
def progress_shape_errors(progress):
    ...
    errors.extend(closed_mapping_errors(
        contract, "Progress contract", CONTRACT_FIELDS))
...
def task_transition_errors(root, progress, catalog, queue, queue_sha,
                            ...
                            items_by_id, coverage, maintenance_context):
    ...
    if task_state == "completion-candidate":
        ...
        # The consumed gate remains current-contract history after the task
        # advances. Retired producer versions are not valid history here.
    ...
    if task_state == "complete" and completion_semantics == "build":
        ...
        # The consumed proof remains current-contract history after the task
        # advances; retired proof formats are outside this runtime.
        errors.extend(historical_receipt_identity_errors(
    ...
    if task_state == "complete" and completion_semantics == "maintenance":
        ...
        # The consumed gate remains current-contract history; exact producer
        # identity and Gate ID are part of that same machine shape.
        ...
        if gate is not None:
            ...
            evidence_errors, expected_context = \
                maintenance_completion_gate_errors(
                    ...
                    allow_complete=True,
                    lifecycle=HISTORICAL_REPLAY,
                )
            errors.extend(evidence_errors)
            errors.extend(maintenance_gate_time_errors(
                maintenance_context, gate,
                lifecycle=HISTORICAL_REPLAY,
            ))
    ...
    for item_id, receipt_id, receipt in transitions:
        ...
        transition_class = (
            runtime_state_contract.classify_queue_transition(
                replay_protocol, before_state, after_state,
                before_hold, after_hold, historical=True)
            if replay_protocol is not None else None
        )
        if before_state == after_state and before_hold == after_hold:
            errors.append("transition receipt %s is a state/hold no-op" %
                          receipt_id)
        elif before_state == after_state and before_state in TERMINAL_STATES:
            errors.append("transition receipt %s mutates terminal history" %
                          receipt_id)
        elif transition_class is None:
            errors.append("transition receipt %s has illegal lifecycle edge "
                          "or hold transition %r/%r -> %r/%r" %
                          (receipt_id, before_state, before_hold,
                           after_state, after_hold))

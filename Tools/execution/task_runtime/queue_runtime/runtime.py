...
from Tools.execution.task_runtime.queue_runtime.adoption import standards_adoption_errors
from Tools.execution.task_runtime.queue_runtime.amendments import (
...
from Tools.execution.task_runtime.queue_runtime.close_gate import close_gate_reuse_errors
from Tools.execution.task_runtime.queue_runtime.control_plane import (
    hub_page_admission,
    profile_hub_paths,
)
...
from Tools.execution.task_runtime.queue_runtime.item_evidence import item_evidence_errors
from Tools.execution.task_runtime.queue_runtime.locks import (
    bind_generic_lock_receipts,
    bind_lock_delta_archives,
    bind_lock_receipts,
    bind_lock_state_phases,
    inventory_writer_locks,
)
...
from Tools.execution.task_runtime.queue_runtime.revalidation import (
...
from Tools.execution.task_runtime.queue_runtime.task_contract import (
    initial_task_plan_receipt_errors,
)
from Tools.execution.task_runtime.queue_runtime.work_spec import work_spec_errors
...
QUEUE_TOP_LEVEL_FIELDS = frozenset((
    ...
    "state_revision", "upstream_revision_id", "selected_profile_manifest",
    "required_queue",
))
...
def validate_runtime(root, allowed_open_delta=None,
                     ...
                     *, gate_evidence_errors,
                     pending_active_standards_receipt_id=None):
    ...
    if (pending_active_standards_receipt_id is not None and
            not nonempty_string(pending_active_standards_receipt_id)):
        raise TypeError(
            "pending_active_standards_receipt_id must be a non-empty "
            "Receipt ID")
    ...
    # The pending-lineage check and the catalog materializer must inspect the
    # same exact set even if an internal caller supplied a one-shot iterable.
    extra_receipts = tuple(extra_receipts or ())
    pending_standards_receipts = [
        receipt for receipt in extra_receipts
        if isinstance(receipt, dict) and
        receipt.get("receipt_id") == pending_active_standards_receipt_id
    ] if pending_active_standards_receipt_id is not None else []
    if (pending_active_standards_receipt_id is not None and
            len(pending_standards_receipts) != 1):
        raise ValueError(
            "pending_active_standards_receipt_id must identify exactly one "
            "extra Receipt")
    root = os.path.realpath(os.path.abspath(root))
    errors = []
    writer_lock_records = inventory_writer_locks(root, errors)
    try:
    ...
    for data, label in ((queue, "Queue"), (coverage, "Coverage"),
        ...
        if data.get("schema_version") != 2:
            errors.append("%s schema_version must be 2" % label)
    ...
    if allow_active_standards_mismatch_for_adoption:
        ...
        active_standards_view, active_errors = \
            active_standards_authorized_view(
                root, queue.get("upstream_revision_id"),
    ...
    bind_lock_delta_archives(root, writer_lock_records)
    bind_generic_lock_receipts(root, writer_lock_records, catalog)
    receipt_type_registry = getattr(catalog, "_type_registry", None)
    if receipt_type_registry is None:
        try:
            receipt_type_registry = \
        ...
        for lifecycle in ("hot", "historical"):
            admission_errors.extend(
                ...
                for error in receipt_type_contract.current_receipt_errors(
                    ...
                    registry=receipt_type_registry))
        if admission_errors:
            errors.extend(
                "pending receipt is not a current-contract Receipt: %s" %
                error for error in admission_errors)
            continue
    ...
    if active_standards_view is not None:
        ...
        if (pending_active_standards_receipt_id is not None and
            ...
            errors.append(
                "pending active Standards Receipt does not match the "
                "proposed latest_adoption_receipt")
        errors.extend(adoption_lineage_contract.current_lineage_errors(
            active_standards_view,
            profile_evidence=profile_evidence,
            catalog=catalog,
            root=root,
            pending_receipt_id=pending_active_standards_receipt_id,
        ))
    ...
    for index, item in enumerate(items):
        ...
        for object_path in manifest:
            ...
            path_error_message = repository_path_refusal(
                root, object_path, must_exist=False)
    ...
    if task_state == "complete" and remaining > 0:
        errors.append("Progress task_state=complete but %d Required work unit(s) remain" %
                      remaining)
    maintenance_context = MaintenanceConsumerContext.from_parts(
        root, {
            "progress": progress,
            "coverage": coverage,
            "queue": queue,

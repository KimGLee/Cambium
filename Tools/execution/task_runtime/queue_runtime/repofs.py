    ...
    if overrides and relative_path in overrides:
        ...
        if not isinstance(raw, bytes) or not isinstance(data, dict):
            raise ValueError("invalid in-memory state override for %s" %
                             relative_path)
        # A proposed initial runtime has no live state files yet.  The
        # override is still path-checked against the registered state
        # namespace, but its whole purpose is to validate candidate bytes
        # without first publishing them.
...
def repository_path_refusal(root, raw_path, must_exist=False):
    """Return why a declared path is inadmissible, or ``None`` if valid.

    The path must stay inside ``root``.  When ``must_exist`` is true, it must
    also resolve to a regular file.  Callers retain ownership of the context
    around that refusal; this function owns only the shared path verdict.
    """
    try:

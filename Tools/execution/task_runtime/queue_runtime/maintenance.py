...
import Tools.execution.task_runtime.queue_runtime.evidence_identity as evidence_identity
...
from Tools.execution.task_runtime.queue_runtime.canon import (
    ...
    TASK_TRANSITION_TOOL,
    TASK_TRANSITION_TOOL_VERSION,
    TERMINAL_STATES,
    TOOL,
    TOOL_VERSION,
    WATERMARK_PATH,
...
from Tools.execution.task_runtime.queue_runtime.repofs import repository_evidence_file
from Tools.execution.task_runtime.queue_runtime.task_contract import contract_sha256
from Tools.execution.task_runtime.queue_runtime.task_record import (
    pending_control_ids,
...
CURRENT_AUTHORIZATION = \
    evidence_identity.EVIDENCE_USE_CURRENT_AUTHORIZATION
HISTORICAL_REPLAY = "historical-replay"


@dataclass(frozen=True)
class MaintenanceConsumerContext:
    """One explicit authority context for the whole maintenance lifecycle.

    ``current_receipts`` is the only view allowed to authorize a new Gate or
    task transition. ``historical_receipts`` is the only view allowed to
    replay already-persisted predecessor and transition chains.  Both are
    required; an absent current view must never fall back to history.
    """

    root: str
    runtime: dict
    current_receipts: CurrentReceiptCatalog
    historical_receipts: HistoricalReceiptCatalog

    @classmethod
    def from_runtime(cls, result):
        if not isinstance(result, dict):
            raise TypeError("maintenance consumer runtime must be a mapping")
        missing = [
            field for field in (
                "root", "current_receipt_catalog", "receipt_catalog",
            ) if field not in result
        ]
        if missing:
            raise ValueError(
                "maintenance consumer runtime lacks explicit view(s): %s" %
                ", ".join(missing)
            )
        invalid = [
            field for field in (
                "current_receipt_catalog", "receipt_catalog",
            ) if not isinstance(result.get(field), dict)
        ]
        if invalid:
            raise TypeError(
                "maintenance consumer view(s) must be mappings: %s" %
                ", ".join(invalid)
            )
        root = result.get("root")
        if not isinstance(root, str) or not root:
            raise ValueError("maintenance consumer runtime has no root")
        return cls(
            root=root,
            runtime=result,
            current_receipts=current_receipt_catalog(result),
            historical_receipts=historical_receipt_catalog(result),
        )

    @classmethod
    def from_parts(cls, root, runtime, *, current_receipts,
                   historical_receipts):
        """Build the validator-owned context before its result is returned."""
        if not isinstance(runtime, dict):
            raise TypeError("maintenance runtime parts must be a mapping")
        candidate = dict(runtime)
        candidate["root"] = root
        candidate["current_receipt_catalog"] = current_receipts
        candidate["receipt_catalog"] = historical_receipts
        return cls.from_runtime(candidate)

    def catalog_for(self, lifecycle):
        if lifecycle == CURRENT_AUTHORIZATION:
            return self.current_receipts
        if lifecycle == HISTORICAL_REPLAY:
            return self.historical_receipts
        raise ValueError("unknown maintenance receipt lifecycle %r" % lifecycle)

    def with_pending_current(self, records):
        """Return a proposed authorization view without rewriting history."""
        current = copy.copy(self.current_receipts)
        for record in records:
            receipt_id = record.get("receipt_id") if isinstance(record, dict) \
                else None
            if not nonempty_string(receipt_id):
                raise ValueError("pending maintenance Receipt has no receipt_id")
            if receipt_id in current or receipt_id in self.historical_receipts:
                raise ValueError(
                    "pending maintenance receipt_id already exists: %s" %
                    receipt_id
                )
            current[receipt_id] = ("<pending-write>", record)
        return MaintenanceConsumerContext(
            root=self.root,
...
def _maintenance_evidence_receipt(context, receipt_id, label, expected,
                                  path_field, sha_field, errors, *, lifecycle):
    """Validate one maintenance input in its declared authority scope."""
    receipt = require_receipt(
        context.catalog_for(lifecycle), receipt_id, label, errors,
        expected=expected,
    )
    if receipt is None:
        return None
    ...
    absolute = repository_evidence_file(
        context.root, relative_path, "%s %s" % (label, path_field), errors,
    )
...
def _canonical_maintenance_completion_consumers(context, gate_id, gate,
                                                 errors):
    ...
    """
    catalog = context.historical_receipts
    consumers = []
    for consumer_id, (relative, candidate) in sorted(catalog.items()):
        if not isinstance(candidate, dict):
            continue
        if not (candidate.get("tool") == TASK_TRANSITION_TOOL and
                candidate.get("check") == TASK_TRANSITION_CHECK and
        ...
        consumer = require_receipt(
            ...
            expected={
                "tool": TASK_TRANSITION_TOOL,
                "tool_version": TASK_TRANSITION_TOOL_VERSION,
                "check": TASK_TRANSITION_CHECK,
...
def latest_consumed_maintenance_gate(context, contract,
                                      *, current_task_id,
    ...
    for gate_id, (relative, gate) in sorted(catalog.items()):
        ...
        if not (gate.get("tool") == TOOL and
                ...
                gate.get("upstream_revision_id") ==
                contract.get("upstream_revision_id") and
        ...
        claims = [
            ...
            candidate.get("tool") == TASK_TRANSITION_TOOL and
            candidate.get("check") == TASK_TRANSITION_CHECK and
            candidate.get("evidence_receipt") == gate_id
        ]
        if not claims:
        ...
        candidate_errors, candidate_context = \
            ...
            context.root, records, validate_prior=False,
        ...
        consumers = _canonical_maintenance_completion_consumers(
            context, gate_id, gate, local_errors,
        )
        if len(consumers) != 1:
            local_errors.append(
...
def previous_maintenance_candidate_state(context, receipt_id,
                                          contract, errors):
    ...
    receipt = require_receipt(
        context.historical_receipts, receipt_id,
    ...
    if receipt is None:
        return None, [], None
    entry = context.historical_receipts.get(receipt_id)
    ...
    consumers = _canonical_maintenance_completion_consumers(
        context, receipt_id, receipt, errors,
    )
    if len(consumers) != 1:
        errors.append(
...
def maintenance_completion_gate_errors(context, budget_manifest_receipt,
                                        ledger_advance_receipt,
                                        watermark_advance_receipt,
                                        *, allow_complete=False,
                                        lifecycle=CURRENT_AUTHORIZATION):
    """Prove one bounded maintenance run against current canonical state."""
    if not isinstance(context, MaintenanceConsumerContext):
        raise TypeError(
            "maintenance completion requires MaintenanceConsumerContext")
    ...
    budget = _maintenance_evidence_receipt(
        context, budget_manifest_receipt, "maintenance budget manifest",
    ...
    ledger = _maintenance_evidence_receipt(
        context, ledger_advance_receipt, "maintenance Ledger advance",
    ...
    watermark = _maintenance_evidence_receipt(
        context, watermark_advance_receipt,
        ...
        if absolute is not None:
            ...
            if isinstance(manifest, dict):
                ...
                if manifest.get("schema_version") != 3:
                    errors.append(
                        "maintenance budget manifest schema_version must be 3"
                    )
                ...
                (previous_candidate_receipt, previous_candidates,
                 previous_candidate_sha) = \
                    previous_maintenance_candidate_state(
                        context, previous_completion_id, contract, errors,
                    )
                candidate_errors, candidate_context = \
                    maintenance_candidates.validate_candidates(
...
def maintenance_gate_time_errors(context, gate,
                                 *, lifecycle=CURRENT_AUTHORIZATION):
    """Require the gate to follow every terminal event and consumed receipt."""
    if not isinstance(context, MaintenanceConsumerContext):
        raise TypeError(
            "maintenance gate timing requires MaintenanceConsumerContext")
    result = context.runtime
    catalog = context.catalog_for(lifecycle)
    errors = []
    gate_time = timestamp_value(gate.get("checked_at"))
    ...
    for field in ("budget_manifest_receipt", "ledger_advance_receipt",
                  "watermark_advance_receipt"):
        entry = catalog.get(gate.get(field))

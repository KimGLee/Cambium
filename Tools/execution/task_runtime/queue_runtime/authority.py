...
def runtime_authority_context(result):
    ...
    expected_standards = queue.get("upstream_revision_id")
    ...
    if active_view.get("upstream_revision_id") != expected_standards:
        raise ValueError(
...
def runtime_authority_lock_fields(context):
    """Project authority admission and Standards before-image lock fields."""
    runtime_authority_validation_kwargs(context)
    fields = {}
    for spec in RUNTIME_AUTHORITY_REGISTRY:
        fields.update(_LOCK_FIELD_PROJECTORS[spec.authority_id](
            context[spec.context_key]))
    # These positions intentionally remain separate.  The first proves which
    # Standards authority admitted the transaction; the second is the exact
    # transaction before-image consumed by interrupted-write recovery.
    fields["before_standards_state_sha256"] = \
        fields["active_standards_sha256"]
    return fields

...
from Tools.execution.task_runtime.queue_runtime.item_history import latest_merge_transition
...
from Tools.execution.task_runtime.queue_runtime.receipts import delta_gate_receipt_ids, require_receipt
...
def current_candidate_binding(root, result, item, *, allow_absent=False):
    """Return the one current canonical open-batch Delta binding.

    The admitted runtime owns candidate classification; this function only
    read-backs the exact bytes and page evidence IDs that downstream review
    consumers bind. It is shared so a producer and the Runner cannot derive
    different candidate identities from the same runtime snapshot.
    """
    item_id = item.get("id") if isinstance(item, dict) else None
    if not isinstance(item_id, str) or not item_id:
        raise ValueError("candidate Delta requires a Queue item identity")
    ...
    return {
        "path": relative,
        "sha256": sha256,
        "page_receipt_ids": delta_gate_receipt_ids(delta),
...
SETTLEMENT_BINDING_FIELDS = tuple(
    field for field in batch_settlement.TRANSITION_BINDING_FIELDS
...
def closed_delta_apply_errors(item, transition, catalog, queue, root=None):
    ...
    errors.extend(delta_property_event_errors(
        receipt, "%s delta application receipt %s" % (item_id, receipt_id)))
    if root is None:
        errors.append(
            "%s delta application cannot validate property invalidation "
            "without repository root" % item_id)
    else:
        errors.extend(delta_property_invalidation_errors(root, receipt))
    opening_errors, _opening_before = delta_opening_semantic_binding(
        receipt, catalog,
        "%s delta application receipt %s" % (item_id, receipt_id),
        expected_item=item)
    ...
    if (not isinstance(merge_transition, dict) or
            ...
            UPDATE_QUEUE_TOOL_VERSION):
        errors.append(
            "%s delta application has no current frozen merge-ready "
            "settlement" % item_id)
    else:
        for field in SETTLEMENT_BINDING_FIELDS:
            if receipt.get(field) != merge_transition.get(field):
                errors.append(
                    "%s delta application settlement %s does not match the "
                    "frozen merge-ready transition" % (item_id, field))
...
def delta_handoff_errors(relative, delta, item, coverage_records,
    ...
    """
    item_id = item.get("id")
    # The planning contract is the single owner for operation-wide Delta
    # policy. Runtime validation consumes that owner instead of maintaining a
    # second allow/deny list beside the writer and merge-ready transition.
    errors = list(coverage_delta.delta_policy_errors(delta))
...
def delta_apply_receipt_candidates(item, catalog, queue, queue_sha,
    ...
    for receipt_id in sorted(catalog):
        ...
        if not (receipt.get("tool") == APPLY_DELTA_TOOL and

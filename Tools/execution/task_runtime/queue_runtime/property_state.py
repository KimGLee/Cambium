...
The Coverage metadata owner/evidence/projection loop and the semantic
before-set each in-flight change is measured against.  A property is earned
by a named completed event or it is not earned;
...
def _current_inflight_semantic_baselines(
    ...
    Missing or invalid current openings and overlapping active manifests grant no
...
def _content_change_property_evidence_errors(
    ...
    """Bind a live content property to one completed current-contract event."""
...
def review_property_evidence_errors(
    ...
    """Bind ``last_reviewed`` to one completed current-contract review."""
...
def coverage_property_state_errors(
    ...
    page with no earned owner values yet (``property_state: {}``).
    """
    ...
    for index, row in enumerate(pages):
        ...
        try:
            ...
            page_snapshot, semantic_fingerprint, records = \
                metadata_property_state.validate_owner_property_records(
                    ...
                    accepted_stale_fingerprint=inflight_baselines.get(path))
        ...
        if page_has_frontmatter:
            try:
                page_fields = metadata_page_state_contract.frontmatter_mapping(
                    page_text, path)
...
def delta_property_event_errors(receipt, label):
    ...
    Historical replay preserves the Receipt's recorded contract identity; exact
    ...
    if not isinstance(rule_fingerprint, str) or not SHA256_RE.fullmatch(
            rule_fingerprint):
        errors.append("%s has invalid metadata rule fingerprint" %
                      label)
            ...
            for record_index, record in enumerate(invalidation_records):
                ...
                if (not isinstance(owner, dict) or set(owner) !=
                    ...
                    if not nonempty_string(owner.get("evidence_receipt")):
                        ...
                        record_receipts.append(owner["evidence_receipt"])
...
def delta_property_invalidation_errors(
    ...
    The exact invalidated set is frozen event data: every stale content-bound
    owner other than LCM must occur in the event record.  Historical replay
    does not compose today's Profile.  A live after-image may additionally be
    supplied to prove the declared tombstone/removals landed.
    """
    errors = []
    if not SHA256_RE.fullmatch(str(
            receipt.get("metadata_execution_rule_fingerprint") or "")):
        errors.append(
            "property invalidation receipt has no metadata rule "
            "fingerprint")
    ...
    for event in receipt.get("property_events") or []:
        ...
        if (isinstance(review, dict) and
                review.get("content_fingerprint") != after_fingerprint):
            expected_records.append({
                "field": metadata_property_state.LAST_REVIEWED,
                "action": "tombstone-current-owner",
                "before_owner_record": copy.deepcopy(review),
            })
...
def current_open_semantic_baseline_errors(
    ...
    The current producer's before-set lets ``apply_delta`` distinguish a real
    semantic edit from a first observation or a machine-projection-only
    rewrite.  While the batch remains open or merge-ready,
    ``require_live_authority`` additionally binds that active execution
    baseline to the live Profile and metadata implementation.  A terminal
    batch keeps the exact same current-contract shape as immutable history.
    """
    ...
    errors = []
    if (transition.get("tool") != "update_queue" or
            transition.get("tool_version") != UPDATE_QUEUE_TOOL_VERSION):
        return [
            "%s must use the current update_queue producer %r/%r, found "
            "%r/%r" % (
                label, "update_queue", UPDATE_QUEUE_TOOL_VERSION,
                transition.get("tool"), transition.get("tool_version"))
        ]
...
def current_opening_semantic_context(result, item_id):
    ...
    receipt catalog, rejects a non-current producer instead of treating apply-time
    ...
    if (opening.get("tool") != "update_queue" or
            opening.get("tool_version") != UPDATE_QUEUE_TOOL_VERSION):
        raise ValueError(
            "Queue item %s latest opening receipt uses a non-current producer "
            "%r/%r; "
...
def current_close_transition_metadata_errors(
    ...
    A current close is the durable bridge from the exact batch-close
    page-review children to the Coverage owner state it published, so its
    child set and recorded metadata-contract identity must be closed and
    exact.  The transition and its close Gate must agree with each other; a
    terminal edge is not reinterpreted through today's implementation bytes.
    """
    if not isinstance(transition, dict) or not (
    ...
    aggregate = None
    resolve_reference = getattr(catalog, "resolve_reference", None)
    if callable(resolve_reference) and nonempty_string(close_id):
        try:
            resolved = resolve_reference(
                close_id, "queue-transition.close-gate")
            aggregate = resolved.body if resolved is not None else None
        except (OSError, UnicodeError, ValueError) as exc:
            errors.append(
                "%s cannot resolve close Gate %s: %s" %
                (label, close_id, exc))
    elif not callable(resolve_reference):
        errors.append(
            "%s receipt catalog does not provide the current typed resolver" %
            label)

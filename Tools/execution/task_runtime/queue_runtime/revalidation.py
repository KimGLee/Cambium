...
from Tools.execution.task_runtime.queue_runtime.item_history import (
...
def standards_revalidation_requirements(root, progress, capabilities=None,
    ...
    for record in records:
        ...
        try:
            ...
            continue
        # Current plans freeze owner Gate IDs in required_gate_ids. Historical
        # replay consumes that frozen closure and never remaps it through a
        # later capability table.
        use_live_leaf_projection = False
        boundaries = {
        ...
        for boundary_id, batch_ids in target_batches.items():
            ...
            for batch_id in batch_ids:
                ...
                # The frozen owner closure is authoritative for replay.
...
def consumed_standards_revalidation_keys(item, catalog):
    ...
    # test context that passes a plain dict keeps the current lookup behavior.
    resolve = getattr(catalog, "resolve", None)
    if not callable(resolve):
        resolve = catalog.get
    ...
    for transition in transitions:
        ...
        # A consumed aggregate remains current-contract history after it leaves
        # the hot authorization set.  It must retain the same producer identity;
        # an adoption that retracts it names it in `invalidated_evidence`.
...
def outstanding_standards_revalidation(result, batch_id):
    ...
    if not isinstance(requirements, dict):
        # Public helpers also accept deliberately reduced contexts.  Such a
        # caller has no validation-scoped derivation to reuse, so compute the
        # same current-contract projection directly instead of requiring a
        # private field.  ``validate_runtime`` always supplies the derived map
        # once.
    ...
    # Consumption is read from the current-contract history catalog so a
    # same-contract receipt that lost live authority remains an immutable
    # record. Retired producer versions never enter either catalog.
...
def current_attempt_evidence_barrier(result, batch_id):
    ...
    # Activation, confirmation, and hold-clear evidence recorded before this
    # adoption remain immutable current-contract history after the dedicated
    # Standards-revalidation aggregate has been consumed. They are still
    # consumers for adoption-scope
...
def standards_revalidation_receipt_errors(result, batch_id, receipt_id):
    ...
    receipt = require_receipt(
        ...
        expected={
            ...
            "upstream_revision_id": (result.get("queue") or {}).get(

...
``producer_module`` resolves a producer through the top-level adapter's
machine-readable implementation edge.  Runtime consumers therefore inspect
the unique implementation owner and never require a public adapter to
re-export business constants.
"""
...
from Tools.governance.control.control_registry_contract import (
...
def producer_module(tool):
    ...
    module = None
    try:
        module = load_tool_implementation(tool)
    except (EntrypointResolutionError, ImportError, OSError, ValueError):
        # A missing or broken installed producer is reported by the caller as
        # an identity error.  It must never be inferred from a similarly named
        # package module.
        module = None
...
def receipt_matches_gate_id(receipt, gate_id, registry, dimension=None):
    ...
    if not isinstance(receipt, dict) or not isinstance(predicate, dict):
        return False
    # A revalidation aggregate never infers identity from descriptive fields.
    # Every current-contract input binds the registry key and its one exact
    # producer tuple explicitly; retired receipt formats are outside the
    # runtime rather than a fallback branch here.
    if receipt.get("gate_id") != gate_id:
        return False
    if receipt.get("tool") != predicate["tool"]:
        return False
    if receipt.get("tool_version") != predicate["tool_version"]:
        return False
    if receipt.get("check") != predicate["check"]:
        return False

...
def operational_amendment_registration_errors(
    ...
    if not SHA256_RE.fullmatch(str(
            amendment.get("amendment_impact_sha256") or "")):
        errors.append(
            "%s current registration has invalid "
            "amendment_impact_sha256" % label)
...
def _contract_amendment_row_errors(root, amendment, label, historical_catalog,
    ...
    receipt = entry[1]
    bindings = {
        "tool": CONTRACT_AMENDMENT_TOOL,
        "tool_version": CONTRACT_AMENDMENT_TOOL_VERSION,
    ...
    if (not isinstance(changed, list) or not changed or
        ...
        errors.append(
            "%s changed_contract_fields must be a non-empty sorted subset "
            "of policy_exceptions/amendment_authority" % label)
    ...
    for field in runtime_state_contract.RUNTIME_STANDARDS_IDENTITY_FIELDS:
        if not nonempty_string(receipt.get(field)):
            errors.append(
                ...
                "upstream revision cannot be identified is not replayable evidence" %
                (label, field))
    # Nonemptiness is not identity: the claimed upstream revision must be one this
...
def queue_replan_amendment_errors(
    ...
    for index, amendment in enumerate(amendments):
        ...
        receipt = require_receipt(
            ...
            expected={
                "tool": COMPILE_QUEUE_TOOL,
                "tool_version": COMPILE_QUEUE_TOOL_VERSION,
                "check": "queue_replan",
                "target": QUEUE_PATH,
                "task_id": queue.get("task_id"),

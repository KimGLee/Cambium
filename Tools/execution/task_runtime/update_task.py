...
TERMINAL_PROOF_TOOL = terminal_proof_contract.PRODUCER_TOOL
TERMINAL_PROOF_TOOL_VERSION = terminal_proof_contract.PRODUCER_TOOL_VERSION
TERMINAL_PROOF_GATE_ID = terminal_proof_contract.GATE_ID
...
TASK_TRANSITION_RECEIPT_TYPE_ID = \
    runtime_state_contract.TASK_TRANSITION_REPLAY_PROTOCOL
TASK_TRANSITION_ABORT_RECEIPT_TYPE_ID = "task-transition-abort-receipt-v1"


def current_task_transition_receipt_errors(record, *, root=None):
    errors = receipt_type_contract.base_receipt_errors(
        record,
        receipt_type_id=TASK_TRANSITION_RECEIPT_TYPE_ID,
        tool=TOOL,
        tool_version=TOOL_VERSION,
        checks=TASK_TRANSITION_CHECK,
    )
    if isinstance(record, dict):
        for field in ("before_task_state", "after_task_state",
                      "before_progress_sha256", "after_progress_sha256"):
            if field not in record:
                errors.append("task transition Receipt misses %s" % field)
    return errors


def current_task_transition_abort_receipt_errors(record, *, root=None):
    errors = receipt_type_contract.base_receipt_errors(
        record,
        receipt_type_id=TASK_TRANSITION_ABORT_RECEIPT_TYPE_ID,
        tool=TOOL,
        tool_version=TOOL_VERSION,
        checks="task_transition_abort",
    )
    if isinstance(record, dict) and not isinstance(
            record.get("aborted_task_transition_receipt"), str):
        errors.append("task transition abort Receipt has no aborted Receipt")
    return errors
...
def _historical_receipt(result, receipt_id, label, expected):
    ...
    """
    catalog = queue_runtime.historical_receipt_catalog(result)
    entry = catalog.get(receipt_id)
    if entry is None:
        raise ValueError("%s receipt %r does not exist" % (label, receipt_id))
    receipt = entry[1]
...
def _latest_transition_timestamp(result, progress):
    ...
    if history is None:
        history = []
    if not isinstance(history, list) or not all(nonempty_string(value)
    ...
    for receipt_id in history:
        ...
        if not queue_runtime.valid_timestamp(checked_at):
            raise ValueError("task transition %s has invalid checked_at" %
                             receipt_id)
        checked_time = queue_runtime.timestamp_value(checked_at)
...
def _terminal_proof_receipt(result, receipt_id):
    ...
    receipt = _receipt(result, receipt_id, "Terminal Proof", {
        ...
        "check": terminal_proof_contract.GATE_CHECK,
        "task_id": progress.get("task_id"),
    ...
    if not nonempty_string(proof_path) or not queue_runtime.SHA256_RE.fullmatch(
            str(proof_sha)):
    ...
    for field in profile_contract.PROFILE_LOAD_EVIDENCE_FINGERPRINT_FIELDS:
        if not queue_runtime.SHA256_RE.fullmatch(str(receipt.get(field))):
            raise ValueError(
    ...
    profile_errors = \
        queue_runtime.profile_load_authorized_view_currency_errors(
    ...
    if not queue_runtime.SHA256_RE.fullmatch(str(repository_snapshot)):
        raise ValueError(
...
def _require_transaction_currency(root, authority, phase,
    ...
    queue_runtime.require_runtime_authority_current(root, authority, phase)
...
def _task_transition_abort_receipt(result, transition_receipt):
    ...
    identity = {
        ...
        "upstream_revision_id":
            (progress.get("contract") or {}).get("upstream_revision_id"),
        "selected_profile_manifest":
...
def _maintenance_completion_receipt(result, receipt_id):
    progress = result["progress"]
    consumer_context = queue_runtime.MaintenanceConsumerContext.from_runtime(
        result)
    receipt = _receipt(result, receipt_id, "maintenance completion gate", {
        ...
        "scope_version": (progress.get("contract") or {}).get(
            "scope_version"),
        "upstream_revision_id": (progress.get("contract") or {}).get(
            "upstream_revision_id"),
    ...
    evidence_errors, expected_context = \
        queue_runtime.maintenance_completion_gate_errors(
            consumer_context,
    ...
    time_errors = queue_runtime.maintenance_gate_time_errors(
    ...
    for field in (
        ...
        if (not isinstance(value, list) or
                not all(nonempty_string(item) for item in value) or
...
def build_task_transition(result, after_state, at, summary, evidence_receipt,
    ...
    if completion_semantics not in queue_runtime.COMPLETION_SEMANTICS:
        raise ValueError(
    ...
    at_value = queue_runtime.timestamp_value(at)
    if at_value is None:
    ...
    if isinstance(checkpoint, dict):
        ...
        if recorded_at is not None:
            recorded_value = queue_runtime.timestamp_value(recorded_at)
    ...
    if before_state == "planned" and after_state == "active":
        if not nonempty_string(first_open_batch_id):
            raise ValueError(
    ...
    if after_state == "completion-candidate":
        ...
        if not nonempty_string(evidence_receipt):
        ...
        phase_errors = queue_runtime.task_phase_delivery_errors(
        ...
        completion_time = queue_runtime.timestamp_value(
        ...
        if completion_semantics == "build":
            ...
            if not nonempty_string(terminal_proof_receipt):
            ...
            terminal_time = queue_runtime.timestamp_value(
            ...
            if not nonempty_string(maintenance_completion_receipt):
                raise ValueError(
    ...
    receipt = kblib.make_receipt(
        TOOL, TOOL_VERSION, TASK_TRANSITION_CHECK, progress.get("task_id"),

...
# Local classifiers consume the neutral Tool owners directly. Amendment
# policy classifies authorized changes; it does not acquire document-shape
# ownership merely because it must inspect that shape.
...
IDENTITY_FIELDS = frozenset((
    "schema_version", "task_id", "upstream_revision_id",
...
def amendment_authority_errors(value, label="amendment_authority"):
    ...
    if not nonempty_string(value.get("authority_id")):
...
def _map_by_id(values, field, label):
    ...
    for index, value in enumerate(values):
        if not isinstance(value, dict) or not nonempty_string(value.get(field)):
...
def derive_amendment_impact(current, proposal, queue):
    ...
    for path in sorted(set(current_pages).union(proposed_pages)):
        ...
        if before is None:
            after_form = coverage_contract.classify_page_form(after)
            reasons.extend(coverage_contract.page_shape_errors(
                after, "new Coverage page %s" % path))
            ...
            if after_form != coverage_contract.PAGE_FORM_PLANNING:
                reasons.append(
                    "new Coverage page %s must use the complete planning-only "
                    "form until its Queue batch opens; found %s" %
                    (path, after_form))
...
def resolve_authority(contract, impact, requested_mode="auto",
    ...
    if requested_mode == "auto" and not delegated:
        if nonempty_string(approval_reference):

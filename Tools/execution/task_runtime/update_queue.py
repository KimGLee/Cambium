...
def _pre_apply_coverage_restore(result, apply_receipt):
    ...
    if not nonempty_string(relative) or not nonempty_string(expected_sha):
        raise ValueError(
...
def _current_attempt_revalidation_receipts(item, result):
    ...
    receipts = []
    for transition in queue_runtime.item_revalidation_discharges(item, catalog):
...
def _current_delta_gate_receipts(item, result):
    ...
    return queue_runtime.delta_gate_receipt_ids(kblib.load_yaml_file(delta))


def _sync_progress(progress, queue, queue_text):
    result = copy.deepcopy(progress)
    result["required_queue_path"] = queue_runtime.QUEUE_PATH
...
def _transition_item(item, args, result):
    ...
    if args.transition and after not in \
            runtime_state_contract.BATCH_LIFECYCLE_TRANSITIONS.get(
                before, frozenset()):
    ...
    def require_standards_revalidation():
        ...
        if not outstanding:
            return
        if not nonempty_string(args.standards_revalidation_receipt):
            raise ValueError(
                "batch has outstanding Standards revalidation; supply "
                "--standards-revalidation-receipt from "
                "check_queue --require-revalidation")
        receipt_errors = queue_runtime.standards_revalidation_receipt_errors(
    ...
    if args.transition == "open" and before == "queued":
        ...
        if not nonempty_string(args.gate_receipt):
        ...
        actual_activation_context = card_activation.context_from_receipt(
            activation_receipt)
        # Opening consumes only the current activation contract.  Its Bundle
        # must equal the current Card/Read Set bytes; retired activation
        # formats are outside the runtime input space rather than alternate
        # validation branches.
        ...
        policy_errors = coverage_delta.delta_policy_errors(delta_data)
        try:
            delta_gate_receipts = queue_runtime.delta_gate_receipt_ids(delta_data)
        ...
        if not isinstance(coverage_source, str):
            ...
            try:
                page_text, planned, rejected, _ = \
                    coverage_delta.plan_page_updates(
                        coverage_source, delta_data)
                prospective_text = coverage_delta.project_coverage_text(
        ...
        if not args.batch_receipt or not all(nonempty_string(value)
        ...
        if len(args.batch_receipt) != 1:
            raise ValueError(
                ...
                "batch-review gate")
        batch_receipt_errors = queue_runtime.batch_review_receipt_errors(
        ...
        if replayed:
            raise ValueError("open -> merge-ready reuses invalidated receipt(s): %s" %
                             ", ".join(replayed))
        wrapper_entry = queue_runtime.current_receipt_catalog(result).get(
        ...
        # exact actual set.  A batch with no current review requirements
        # carries no obligations and must carry no judgment bindings —
        # sealed evidence keeps its own shape, exactly like the K12/12
        # guard above.
        judgment_errors = queue_runtime.batch_review_judgment_errors(
            result, item, wrapper_receipt)
        if judgment_errors:
            raise ValueError(
        ...
        phase_errors = queue_runtime.activation_phase_delivery_errors(
        ...
        if queue_runtime.batch_touches_control_plane(item):
            governance_errors = queue_runtime.activation_phase_delivery_errors(
        ...
        if not nonempty_string(args.close_gate_receipt):
            raise ValueError(
        ...
        close_gate_errors = queue_runtime.close_gate_receipt_errors(
            queue_runtime.current_receipt_catalog(result),
        ...
        settlement_errors = queue_runtime.batch_reference_settlement_errors(
            result, item)
        ...
        close_entry = queue_runtime.current_receipt_catalog(result).get(
            args.close_gate_receipt)
        ...
        elif nonempty_string(args.delta_apply_receipt):
            raise ValueError(
    ...
    if args.hold_state is not None and args.transition not in ("open",):
        if args.hold_state not in queue_runtime.HOLDS:
        ...
        if (args.hold_state == "none" and
                queue_runtime.item_undischarged_revalidation_hold(
        ...
        if args.hold_state == "none":
            item.pop("hold_reason", None)
        elif not nonempty_string(args.reason):
...
def main(argv=None):
    ...
    write.add_argument("--transition", choices=tuple(sorted({
        target for targets in
        runtime_state_contract.BATCH_LIFECYCLE_TRANSITIONS.values()
        for target in targets
    })),
                       help="target lifecycle state; exclusive with "
                            "--hold-state")
    write.add_argument("--hold-state", choices=tuple(queue_runtime.HOLDS),
...
def _run(args, produced):
    ...
    if not queue_runtime.valid_timestamp(args.at):
    ...
    try:
        ...
        print("[FAIL] %s" % exc)
        return 1
    transition_class = runtime_state_contract.classify_queue_transition(
        runtime_state_contract.ORDINARY_QUEUE_TRANSITION_CAPABILITY,
        before_state, after_state,
        item.get("hold_state"), target.get("hold_state"),
    )
    if transition_class is None:
        print("[FAIL] proposed Queue lifecycle/hold edge has no current "
    ...
    receipt = queue_runtime.make_queue_receipt(
    ...
    if not delta_move:
        proposed = runtime_validation.validate_runtime(
            root,
            state_overrides={
                queue_runtime.COVERAGE_PATH: (coverage_text, coverage_new),
    ...
    try:
        lock_operation = {
            ...
            "before_state_revision": before_revision,
            "before_queue_sha256": before_sha,
            "before_coverage_sha256": before_coverage_sha,
            ...
            "planned_after_queue_sha256": after_sha,
        ...
        with kblib.runtime_write_lock(root, owner_metadata=lock_operation) as lock:
            ...
            with kblib.no_authoritative_write_guard(lock):
                ...
                if args.transition == "closed":
                    ...
                    locked_close_errors = queue_runtime.close_gate_receipt_errors(
                        queue_runtime.current_receipt_catalog(current),
                        args.close_gate_receipt,
                        item_id=args.id,
                ...
                if args.transition == "merge-ready":
                    ...
                    (locked_page_text, locked_planned,
                     locked_rejected, _) = \
                        coverage_delta.plan_page_updates(
                            old_coverage_text, locked_delta)

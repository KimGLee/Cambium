...
Current authorization and current-contract history validation are deliberately
separate.  Historical validation applies only to receipts produced by the
same current contract after they cease to authorize live state.
"""
...
AMENDMENT_BATCH_CANCELLATION_REPLAY_PROTOCOL = \
    "amendment-queue-transition-receipt-v1"
QUEUE_LIFECYCLE_TRANSITION = "lifecycle"
QUEUE_HOLD_TRANSITION = "hold"
...
def validate_model(document):
    ...
    expected_queue_history = {
        QUEUE_TRANSITION_REPLAY_PROTOCOL,
        AMENDMENT_BATCH_CANCELLATION_REPLAY_PROTOCOL,
    }
    if set(queue_history) != expected_queue_history:
        raise RuntimeStateContractError(
            "queue current-contract history protocols must be %s" %
            ", ".join(sorted(expected_queue_history)))
    seen_current_edges = {}
    ...
    if set(task_history) != expected_task_history:
        raise RuntimeStateContractError(
            "task current-contract history must use %s for every completion "
...
def load_model(root=None):
    ...
    if root is None:
        root = Path(repository_source_root(__file__))
...
def task_transition_is_authorized(completion_semantics, before, after,
                                  *, historical=False):
    """Test one current writer edge or current-contract historical edge."""
...
def classify_queue_transition(authority_id, before_state, after_state,
                              before_hold_state, after_hold_state, *,
                              historical=False):
    """Classify one current-contract Queue lifecycle or hold transition.

    Queue lifecycle states and hold states are two axes of the same persisted
    transition record.  The Kernel registry enumerates lifecycle edges and
    the complete hold-state vocabulary; a hold-only write therefore keeps a
    nonterminal lifecycle state unchanged while changing to another registered
    hold state.  It is not a missing ``state -> state`` lifecycle edge.

    ``authority_id`` is either a current writer capability or a current-
    contract historical replay protocol.  Cancellation owns lifecycle edges
    only; hold-only transitions remain owned by the ordinary Queue writer.
    Invalid states, invalid holds, terminal mutation, no-ops, unknown owners,
    and edges outside the owner's catalog return ``None``.
    """
    if (before_state not in QUEUE_STATES or after_state not in QUEUE_STATES or
            before_hold_state not in QUEUE_HOLD_STATES or
            after_hold_state not in QUEUE_HOLD_STATES):
        return None
    authorities = (BATCH_HISTORICAL_TRANSITIONS_BY_PROTOCOL
                   if historical else BATCH_TRANSITIONS_BY_CAPABILITY)
    edges = authorities.get(authority_id)
    if edges is None:
        return None
    if before_state != after_state:
        if (before_state, after_state) in edges:
            return QUEUE_LIFECYCLE_TRANSITION
        return None
    ordinary_authority = (QUEUE_TRANSITION_REPLAY_PROTOCOL if historical else
                          ORDINARY_QUEUE_TRANSITION_CAPABILITY)
    if (authority_id != ordinary_authority or
            before_state in QUEUE_TERMINAL_STATES or
            before_hold_state == after_hold_state):
        return None
    return QUEUE_HOLD_TRANSITION
...
BATCH_HISTORICAL_TRANSITIONS_BY_PROTOCOL = MappingProxyType({
    key: record["edges"] for key, record in _MODEL["queue_history"].items()
})

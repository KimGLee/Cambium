...
# One projection owns both the receipt field names and their report sources.
# Validators derive their accepted fields from this table, so extending the
# producer cannot silently leave a consumer on an older allowlist.
TRANSITION_BINDING_SOURCES = (
    ("settlement_protocol", "protocol"),
    ("routed_gap_obligation_count", "obligation_count_before"),
    ("routed_gap_obligation_set_sha256",
     "obligation_set_sha256_before"),
    ("routed_gap_obligation_record_set_sha256",
     "obligation_record_set_sha256_before"),
    ("prospective_unsettled_count", "unsettled_count_after"),
    ("prospective_unsettled_set_sha256",
     "unsettled_set_sha256_after"),
)
TRANSITION_BINDING_FIELDS = tuple(
    output_field
    for output_field, _source_field in TRANSITION_BINDING_SOURCES
)
...
def transition_binding(report):
    return {
        output_field: report[source_field]
        for output_field, source_field in TRANSITION_BINDING_SOURCES
    }

...
# The current protocol freezes the complete deliverable set at admission,
# assigns every frozen file to one declared phase, and delivers one phase at a
# time inside the Tool-owned result budget.  Delivery completion is earned by
# exact phase acknowledgement; admission never asserts that content was read.
...
_DEPLOYED_ROOT = repository_source_root(__file__)
...
ACTIVATION_MANIFEST_FIELDS = frozenset((
    "activation_protocol", "task_id", "batch_id", "upstream_revision_id",
    "selected_profile_manifest", "required_queue_sha256",
    "coverage_ledger_sha256", "progress_ledger_sha256", "queue_revision",
    "queue_state_revision", "active_standards_sha256",
    "profile_snapshot_sha256", "profile_contract_fingerprint",
    "profile_load_inputs_sha256", "task_contract_sha256",
    "card_index_sha256", "reading_plan_sha256", "readback_plan_sha256",
    "reading_plan", "pieces", "piece_count", "max_piece_envelope_bytes",
    "readback_plan", "batch_review_plan", "phase_plan",
    "phase_plan_sha256",
))
...
def _route_registry(root):
    """Project the current Card/Read Set registry for activation delivery."""
    try:
    ...
    result = {
        route_id: {
            "path": cards[route_id]["path"],
            "read_set": read_sets[route_id]["path"],
...
def _delivery_binding(execution_context_id=None):
    ...
    A bound context records only `host-bound`; an unbound CLI result records
    `prepared`.  Completion is earned later by exact phase acknowledgement.
    """
...
def _card_review_hash_matches(record):
    """Validate the current curated Card review proof."""
    ...
    return (record.get("source_hash") == reviewed and
            isinstance(reviewed, str) and
            SHA12_RE.fullmatch(reviewed) is not None and
            isinstance(reviewed_card, str) and
...
def _piece_records(cards, startup, phase_of=None):
    ...
    for row in startup:
        ...
        # Supplemental Profile Read Sets use the current declaration contract
        # and travel as startup material because they have no top-level Card.
...
def build_activation_context(root, progress, item, *, runtime_state,
    ...
    # Packing measures the real serialization and performs the fail-closed
    # budget check: an oversized leaf raises here, at its own
...
def _phase_delivery_payload(manifest, phase_id, part_index, part_count,
    ...
    A part carries a list of whole files and the nonce sits last.  The nonce
    shows the
...
def _pack_phase_parts(manifest, phase_id, records, texts_by_id):
    ...
    # leaf's own governance problem and it fails closed here.  Deciding this
    # before packing keeps an undeliverable leaf from being reported later as
    # a phase that was merely cut too wide.
    oversized = []
...
def phase_record(activation_context, phase_id):
    """Return one frozen phase record, or None when it is absent."""
...
def activation_context_errors(context):
    ...
    if set(context) != set(ACTIVATION_CONTEXT_FIELDS):
        errors.append("activation context fields do not match the current "
                      "closed contract")
    protocol = context.get("activation_protocol")
    if protocol != ACTIVATION_PROTOCOL:
        errors.append("activation_protocol must be %s" % ACTIVATION_PROTOCOL)
        return errors
    manifest = context.get("activation_bundle_manifest")
    if not isinstance(manifest, dict):
        return errors + ["activation_bundle_manifest must be a mapping"]
    if set(manifest) != ACTIVATION_MANIFEST_FIELDS:
        errors.append("activation bundle fields do not match the current "
                      "closed contract")
    ...
    if not isinstance(review_sha, str) or not SHA256_RE.fullmatch(
            review_sha or ""):
        errors.append("review_requirement_set_sha256 must be a sha256 value")
    if not isinstance(plan, dict):
        errors.append("activation bundle batch_review_plan must be a mapping")
    else:
        ...
        if not isinstance(records, list):
            ...
            try:
                ...
                recomputed = None
                errors.append(
                    "batch_review_plan requirements are malformed: %s" % exc)
            if recomputed is not None and (
                    plan.get("review_requirement_set_sha256") != recomputed or
                    review_sha != recomputed):
                errors.append(
                    "review_requirement_set_sha256 does not bind the frozen "
    ...
    if assurance == "host-bound":
        if mode != "host-context-injection" or not isinstance(
            ...
            errors.append("host-bound admission requires one execution context")
    elif assurance == "prepared":
        if mode != "cli-tool-result" or context_id is not None:
            errors.append("prepared admission must be an unbound CLI result")
    else:
        errors.append(
...
def _phase_plan_errors(context, manifest):
    ...
    if not isinstance(environment, dict):
        ...
        for field in ("upstream_revision_id", "selected_profile_manifest",
                      "profile_snapshot_sha256",
...
def _piece_manifest_errors(manifest):
    """Validate the current activation's frozen, phase-bound file set."""
    errors = []
...
def activation_receipt_binding(context):
    ...
    return {field: context.get(field)
            for field in ACTIVATION_CONTEXT_FIELDS}
...
def build_phase_delivery(root, activation_context, phase_id, part_index=0, *,
    ...
    for piece_id in part.get("piece_ids") or []:
        ...
        # Re-prove every file against current bytes. Grouping files into one
        # result must not weaken the per-file drift check that makes the frozen
        # SHA meaningful.
    ...
    attempt = delivery_attempt_id or expected_delivery_attempt_id(
        ...
        execution_context_id or os.environ.get(
            agent_interface_contract.EXECUTION_CONTEXT_ENV))
...
def build_phase_ack(delivery_receipt, nonce, *, execution_context_id=None):
    ...
    return {
        ...
        "activation_receipt_id": delivery_receipt.get(
            "activation_receipt_id"),
        "batch_id": delivery_receipt.get("batch_id"),
...
PHASE_RECEIPT_FIELDS = (
    "phase_protocol", "activation_receipt_id", "batch_id",
    "card_bundle_sha256", "phase_plan_sha256", "phase_id", "part_index",
    "part_count", "phase_piece_ids", "phase_envelope_bytes",
    "delivery_attempt_id", "delivery_nonce",
    "delivery_mode", "delivery_assurance", "execution_context_id",
)
PHASE_ACK_RECEIPT_FIELDS = (
    "phase_ack_protocol", "activation_receipt_id", "batch_id",
    "card_bundle_sha256", "phase_plan_sha256", "phase_id", "part_index",
    "part_count", "phase_piece_ids",

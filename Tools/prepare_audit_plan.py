...
_PROFILE_SCAN_EXTENSION = "k12-05-registered-scan"
_BATCH_SCOPE_TARGET = "."
...
def _coverage_tiers(result):
    return {path: row.get("tier")
            for path, row in _coverage_rows(result).items()}


def _profile_volatility_defaults(result):
    """Read only the Vocabulary Extensions bound by the admitted Profile."""
    ...
    try:
        slot_pairs = tuple(view["_manifest_slot_paths"])
        slots = dict(slot_pairs)
    except (KeyError, TypeError, ValueError) as exc:
        raise audit_producer_runtime.AuditProducerError(
            "L-tier substantive-review trigger HOLD: authorized Profile "
            "has no immutable manifest-slot projection") from exc
    if len(slots) != len(slot_pairs):
        raise audit_producer_runtime.AuditProducerError(
            "L-tier substantive-review trigger HOLD: authorized Profile "
            "repeats a manifest-slot identity")
    path = slots.get("Vocabulary Extensions")
    if not isinstance(path, str) or not path:
        raise audit_producer_runtime.AuditProducerError(
            "L-tier substantive-review trigger HOLD: authorized Profile "
            "does not bind Vocabulary Extensions")
    snapshot = view.get("_profile_snapshot")
    try:
        document = kblib.parse_yaml_subset(snapshot.read_text(path))
    except (AttributeError, KeyError, TypeError, UnicodeError, ValueError,
            kblib.YamlSubsetError) as exc:
        raise audit_producer_runtime.AuditProducerError(
            "L-tier substantive-review trigger HOLD: authorized Profile "
            "Vocabulary Extensions cannot be read from its frozen snapshot: "
            "%s" % exc) from exc
    defaults = document.get("volatility_defaults") \
        if isinstance(document, dict) else None
    if (not isinstance(defaults, dict) or
            any(not isinstance(domain, str) or not domain or
                not isinstance(volatility, str) or not volatility
                for domain, volatility in defaults.items())):
        raise audit_producer_runtime.AuditProducerError(
            "L-tier substantive-review trigger HOLD: authorized Profile "
            "volatility_defaults is not a string mapping")
...
def _review_evidence_state(row, page, *, rereview_marked=False,
    ...
    authority here.  Page frontmatter is deliberately not a fallback: a
    legacy marker that has not entered the owner/evidence loop cannot make an
    existing page look reviewed.
    """
...
def _registered_dimensions(base_specs, contract):
...
def _profile_spec(**values):
    """Build one closed spec for compose_profile_extensions to validate."""
    return values


def _profile_extension_specs(root, contract):
    """Project only registrations authorized by the typed Profile."""
    changed = audit_obligation_projection.validate_changed_scope_registry(
        audit_obligation_projection.load_changed_scope_registry(root))
    scan_points = [row for row in changed["extension_points"]
                   if row["extension_point_id"] == _PROFILE_SCAN_EXTENSION]
    ...
    for scan in getattr(contract, "registered_scans", ()):
        if scan.required_for_k12_item_6:
            continue
        judgment = _judgment(contract, scan.judgment_item_id)
        specs.append(_profile_spec(
            ...
            owner_rule_id=scan.scan_id,
            kernel_extension_point=_PROFILE_SCAN_EXTENSION,
            target_source="changed-scope",
            tier=None,
            applicability=scan.candidate_predicate,
            partition="changed-scope-deterministic",
            trigger_partition_mappings=(),
            due_stage=scan_point["due_stage"],
            evidence_role=scan_point["evidence_role"],
            evidence_kind=scan_point["evidence_kind"],
            dimension=judgment.dimension_id,
            dimension_binding="fixed",
            acceptance_predicate=judgment.judgment_item_id,
            producer_capability=scan_point["producer_capability"],
            producer_gate_id=scan_point["producer_gate_id"],
            producer_check=scan.scan_id,
            consumer_gate_id=scan_point["consumer_gate_id"],
            fingerprint_binding="evidence-time",
            nonblocking=scan_point["nonblocking"],
        ))
    ...
    for requirement in getattr(contract, "batch_review_requirements", ()):
        ...
        specs.append(_profile_spec(
            ...
            source_registry=(getattr(contract, "routing_registry_path", None)
                             or contract.manifest_repo_path),
...
def _required_obligation(definition):
    identity = kblib.sha256_bytes(kblib.canonical_json_bytes(definition))
    row = deepcopy(definition)
    row.update({
        "obligation_id": "audit-obligation-%s" %
            identity.split(":", 1)[1][:24],
        "review_due": None,
        "status": "required",
        "evidence_ref": None,
...
def _resolve(spec, target, *, trigger=None, dimension=None,
    ...
    return _required_obligation(definition)
...
def _changed_scope_targets(spec, *, item, frozen, coverage):
    ...
    gap_by_applicability = {
        ...
        "batch-contract-includes-standards-migration":
            "Work Spec has no typed standards-migration applicability flag",
...
def _project_obligations(root, result, item, frozen, generated_at,
    ...
    base_specs = audit_obligation_projection.base_obligation_specs(root)
    ...
    dimensions = _registered_dimensions(base_specs, contract)
    profile_specs = _profile_extension_specs(root, contract)
    composed = audit_obligation_projection.compose_profile_extensions(
        base_specs, profile_specs, registered_dimensions=dimensions,
        root=root)
...
def check_queue_opening_context(result, batch_id):
    """Keep the runtime facade as the sole baseline consumer."""
    import check_queue
    return check_queue.current_opening_semantic_context(result, batch_id)

...
- volatility: an explicit valid frontmatter declaration always wins; when a
  domain -> volatility mapping is supplied via --defaults (the canonical
  .cambium/derived/vocab.yaml artifact, or an explicit standalone flat mapping
  file), pages without an explicit declaration fall back to the mapping
  through their `domain`; otherwise (no --defaults, or
...
TOOL = freshness_receipt_contract.TOOL
TOOL_VERSION = freshness_receipt_contract.TOOL_VERSION
RECEIPT_TYPE_ID = freshness_receipt_contract.RECEIPT_TYPE_ID
RECEIPT_CHECKS = (
    freshness_receipt_contract.CANDIDATE_CHECK,
    freshness_receipt_contract.SUMMARY_CHECK,
)


def current_receipt_errors(record, *, root=None):
    """Validate one current Receipt through its unique payload owner."""
    return freshness_receipt_contract.current_receipt_errors(
        record, root=root)
...
def parse_date(value):
    """Parse one strict ISO date through the freshness contract owner."""
...
def load_frontmatter(path, raw_bytes=None):
    ...
    try:
        ...
        return None, True
    if kblib.extract_frontmatter(text) is None:
        return None, False
    try:
        fm = metadata_page_state_contract.frontmatter_mapping(text, path)
    except (ValueError, kblib.YamlSubsetError):
        return None, True
    return fm, False


def _load_standalone_defaults(path, text=None):
    """Load the scanner's explicit flat domain -> volatility adapter.

    Canonical composed vocabulary artifacts are consumed through
    ``compose_vocab.admitted_volatility_defaults``.  This adapter deliberately
    rejects nested Profile or artifact shapes so it cannot become a second
    parser for Profile-owned policy.
    """
    if text is None:
        with open(path, "rb") as handle:
    ...
    if not isinstance(mapping, dict):
        raise ValueError(
            "standalone defaults file must be a flat domain -> volatility "
            "mapping")
    result = {}
    for domain, volatility in mapping.items():
        if isinstance(volatility, (dict, list)):
            raise ValueError(
                "standalone defaults file must be a flat domain -> "
                "volatility mapping")
        volatility = str(volatility)
        if volatility not in vocabulary_contract.REVIEW_INTERVALS_DAYS:
            raise ValueError(
...
def _candidate_details(outcome, as_of):
    ...
    if outcome.kind == freshness_engine.PENDING_FIRST_VERIFICATION:
        interval = vocabulary_contract.REVIEW_INTERVALS_DAYS[
            outcome.volatility]
        if interval is None:
...
def _run(args):
    ...
    if args.defaults:
        try:
            ...
            if defaults_absolute in canonical_spellings:
                ...
                defaults_snapshot, defaults_map, artifact_errors = \
                    compose_vocab.admitted_volatility_defaults(
                        root, args.defaults, defaults_admission)
                if (artifact_errors or defaults_snapshot is None or
                        defaults_map is None):
                    raise ValueError(
                ...
                defaults_text = defaults_bytes.decode(
                    "utf-8", errors="strict")
                defaults_map = _load_standalone_defaults(
                    args.defaults, defaults_text)
    ...
    receipt_contract_errors = []
    for receipt in receipts:
        receipt_contract_errors.extend(
            "%s: %s" % (receipt.get("receipt_id", "<unknown>"), error)
            for error in current_receipt_errors(receipt, root=root)
        )
    if receipt_contract_errors:
        print("check_freshness: refusing non-contract Receipt publication: %s"
              % "; ".join(receipt_contract_errors))
        return 1

...
governance decision promotes to a gate; it adds no alternate field contract.
...
RECEIPT_TYPE_ID = "page-contract-gate-receipt-v1"
...
def current_receipt_errors(record, *, root=None):
    ...
    if isinstance(record, dict) and record.get("gate_id") != GATE_ID:
        errors.append("gate_id must identify page-contract")
    return errors
    ...
    if contract is not None and admission is not None:
        ...
        if scope:
            ...
            for error in scope_errors:
                ...
                findings.add(
                    "page-contract-profile", SCOPE_SLOT, "fail", error)
        ledger_path = os.path.join(root, COVERAGE_LEDGER_PATH)
        if os.path.isfile(ledger_path):
            try:
                ledger = kblib.parse_yaml_subset(
                    kblib.read_text(ledger_path, errors="replace"))
        ...
        for name, rule in sorted(projection_rule_by_field.items()):
            ...
            if policy == \
                ...
                report(
                    "page-contract-metadata-authority",
                    "%s:%s" % (rel, name),
                    "project page writer does not implement reconcile "
                    "policy %r" % policy)

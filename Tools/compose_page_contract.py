...
def compose(root, base_path, rel_path, profile_dir):
    """Return (contract_fields_ordered, provenance, errors)."""
    errors = []
...
def render(ordered, provenance):
    ...
    body = kblib.canonical_yaml(
        {"schema_version": 1, "fields": ordered})
    return "\n".join(lines) + "\n" + body
...
def main(argv=None):
    ...
    ordered, provenance, errors = compose(root, base_path, rel_path,
                                          profile_dir)
    if errors:
        for error in errors:
            print("compose_page_contract: %s" % error)
    ...
    text = render(ordered, provenance)
    if args.check:
        try:
    ...
    print("compose_page_contract: wrote %s (%d field(s))"
          % (output, len(ordered)))
    return 0

...
FILL = [
    ...
    ("registries/audit-dimensions.md", "TODO(profile) — define what a reported candidate means for this repository and which stable dispositions are acceptable.", "The registered scan reports canonical notes that still carry dated-scratch structure outside `Notes/Daily Log`. Each candidate is resolved one of two ways, recorded on the candidate page: the scratch material is moved into the dated entry that owns it, or the page states why that structure is the canonical form for this note."),

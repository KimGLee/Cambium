...
# Scenario templates.
#
# Most tests in this file walk one of a handful of identical lifecycles --
# build the fixture tree, pause the task, open-and-hold B1, adopt SA-001,
# close B1 -- and then assert one property of the result.  Each distinct
# lifecycle is walked exactly once per process into a template tree below.
...
def _build_paused(walker, inherited):
    walker.pause()


def _build_held(walker, inherited):
    return {"invalidated_gate": walker.open_b1_and_hold_for_revalidation()}
...
def _build_adopted(walker, inherited):
    ...
    receipts_existed_after_dry_run = (walker.root / walker.RECEIPTS).exists()
    ...
    return {
        ...
        "receipts_existed_after_dry_run": receipts_existed_after_dry_run,
...
_TEMPLATE_BUILDERS = {
    "base": _build_base,
    "paused": _build_paused,
    "held": _build_held,
    "adopted": _build_adopted,
...
class AdoptedScenarioTests(_TemplateBackedCase):
    # Shared scenario: the paused fixture tree carried through the default
    # SA-001 plan -- a dry run first, then the applied adoption -- walked
    # once into the "adopted" template.  Every test here reads that
    # committed state or the recorded dry-run/apply artifacts and writes
    # nothing, so the class shares the template tree itself.
    TEMPLATE = "adopted"
    SHARE_TEMPLATE = True

    def test_noop_dry_run_and_missing_register_apply(self):
        plan = self.scenario["plan"]
        self.assertEqual(0, self.scenario["dry_run_code"],
                         self.scenario["dry_run_output"])
        self.assertEqual(self.scenario["state_before_dry_run"],
                         self.scenario["state_after_dry_run"])
        self.assertFalse(self.scenario["receipts_existed_after_dry_run"])
...
class AdoptedHistoryRewriteTests(_TemplateBackedCase):
    # Shared scenario: the same committed SA-001 adoption the "adopted"
    # template walks for AdoptedScenarioTests.  These tests then restamp
    # sealed receipts or poison the selected Profile closure, so each takes
    # a private copy of the template instead of sharing the tree.
    TEMPLATE = "adopted"
...
class PausedPlanAdmissionTests(_TemplateBackedCase):
    # Shared scenario: the fixture tree paused for Standards adoption, the
    # one transition every one of these tests began by making.  Each test
...
class AdoptionTransactionFailureTests(_TemplateBackedCase):
    # Shared scenario: the paused fixture tree, as in
    # PausedPlanAdmissionTests.  Every test injects a write failure or
    # post-admission drift into its own apply attempt and inspects the
...
class CapabilityBoundaryTests(_TemplateBackedCase):
    # Shared scenario: the paused fixture tree.  Each test installs its own
    # closed capability-registry fixture and plan on a private copy; several
    # then apply the adoption and rewrite the sealed artifacts to an older
    # producer shape.
    TEMPLATE = "paused"
...
class ProfileLoadBoundaryTests(_TemplateBackedCase):
    # Shared scenario: the paused fixture tree.  Each test builds its own
    # profile-load boundary plan on a private copy; several install a second
    # Profile package or deliberately break a Profile closure first.
    TEMPLATE = "paused"
...
class HeldRevalidationTests(_TemplateBackedCase):
    # Shared scenario: B1 admitted, opened, and put under the
    # `revalidation-required` hold -- the three-transition walk
    # `open_b1_and_hold_for_revalidation` makes -- performed once into the
    # "held" template, whose invalidated activation receipt rides along as
    # a template artifact.  Every test judges or adopts its own plan against
    # that held position on a private copy of the tree.
    TEMPLATE = "held"

    def setUp(self):
        super().setUp()
        self.invalidated_gate = self.scenario["invalidated_gate"]
    ...
    def test_current_merge_ready_native_owner_requires_rollback(self):
        ...
        invalidated = self.invalidated_gate
        plan = self.capability_boundary_plan(
...
class ReadSetClosureTests(_TemplateBackedCase):
    # Shared scenario: the bare fixture tree exactly as the original setUp
    # built it (task still `planned`), walked once into the "base" template.
    # Each test lays its own Read Set closure fixture -- or breaks the live
    # contract declaration -- on a private copy.
    TEMPLATE = "base"

...
def _canonical_repository_relative_path(relative_path):
    ...
    if relative_path != relative_path.strip():
        raise ValueError("page path must not have surrounding whitespace")
    if "\\" in relative_path:
        raise ValueError("page path must use canonical '/' separators")
    if "\x00" in relative_path:
        raise ValueError("page path must not contain NUL")
    if os.path.isabs(relative_path):
        raise ValueError("page path must be repository-relative")
    parts = relative_path.split("/")
    if any(part in ("", ".", "..") for part in parts):
        raise ValueError(
            "page path must not contain empty, '.' or '..' segments")
    if "/".join(parts) != relative_path:
        raise ValueError(
            "page path must use canonical repository-relative spelling")
...
def page_set_artifact_fingerprint(pages, *, contract=None):
    ...
    for index, pair in enumerate(pages):
        ...
        relative_path = _canonical_repository_relative_path(pair[0])

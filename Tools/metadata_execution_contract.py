...
def compose_profile_projection_rules(contract, profile_contract):
    ...
    if (profile_contract is None or
        ...
        raise ValueError(
            ...
            "Profile contract")
    if not compiled_capability_supports(
            contract, "project-page-state-v2",
            PROFILE_EXTENSION_ENUM_PROJECTION_OPERATION, kind="writer"):
        raise ValueError(
            "installed page writer has no generic Profile enum operation")
    ...
    for index, entry in enumerate(entries):
        ...
        if kind in ("producer", "receipt-schema", "projection") and operations:
            errors.append("%s kind %s must have closed empty operations" %
                          (target, kind))
            continue
        expected = (WRITER_OPERATION_KEYS if kind == "writer" else
                    CONSUMER_OPERATION_KEYS)
        local_seen = set()
...
def load_operation_capabilities(root=None, capabilities_path=None):
    ...
    document = _parse_yaml(repository, path, "operation capabilities")
    errors = _validate_capabilities(document)
    if errors:
        raise MetadataExecutionContractError(errors)
    return document

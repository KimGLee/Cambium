...
def unfilled_sentinel(root):
    ...
    try:
        defaults = kblib.parse_yaml_subset(
            kblib.read_text(os.path.join(
                root, *DEFAULTS_RELATIVE.split("/"))))
...
def corpus_planning_state(root, profile_dir):
    ...
    """
    try:
        manifest_text = kblib.read_text(os.path.join(
        ...
        if kind != "path":
            return "unreadable"
        document = kblib.parse_yaml_subset(kblib.read_text(detail))
...
def candidate_profile_id(root, name):
    ...
    try:
        manifest_text = kblib.read_text(manifest)

...
apply-gated transaction writers produce their receipts inside their own
guarded transactions -- a verification sweep never runs them.  Their
classification is derived from the compiled agent-interface path effects,
not from a second tool-name list.  A registry row this runner recognizes in
none of those classes fails the run closed: the registry grew and this runner
must be extended WITH it, loudly, never silently skipped.
...
SCRIPT_DIR = tools_source_root(__file__)
    ...
    if override:
        ...
        if not exists:
            raise RunnerError(
                ...
                (override, profile_layout_contract.PROFILE_MANIFEST_NAME))
        return manifest
    queue_path = os.path.join(root, queue_runtime.QUEUE_PATH)
...
def _projection_target(manifest):
    location = profile_layout_contract.parse_profile_manifest_path(manifest)
    return (
...
def _preflight_commands(root, manifest):
    ...
    projection_target = _projection_target(manifest)
    python = _python()
    return [
...
def derive_verification_set(root, registry, recipes, transaction_writers):
    ...
    for gate_id, predicate in sorted(registry.items()):
        ...
        tool = predicate["tool"]
        mode = predicate["mode"]
        if tool == queue_runtime.MANUAL_ATTESTATION_TOOL:
...
def main(argv=None):
    ...
    registry, registry_errors = queue_runtime.standards_gate_registry(root)
    registry_errors.extend(
        queue_runtime.gate_registry_producer_errors(registry))
    if registry_errors:
    ...
    try:
        ...
        interface_contract = compile_cli_contract.compile_contract(
            root, _projection_target(manifest))
        transaction_writers = \
            compile_cli_contract.apply_gated_writer_tools(interface_contract)
    except (RunnerError, profile_contract.ProfileContractError,
            compile_cli_contract.ContractError, OSError, ValueError) as exc:
        print("run_gates: %s" % exc)
        return 1

    derived, hard_errors = derive_verification_set(
        root, registry, recipes, transaction_writers)

...
TOOL = adoption_lineage_contract.STANDARDS_ADOPTION_TOOL
TOOL_VERSION = adoption_lineage_contract.STANDARDS_ADOPTION_TOOL_VERSION
GATE_ID = adoption_lineage_contract.STANDARDS_ADOPTION_GATE_ID
...
PLAN_PREFIX = queue_runtime.STANDARDS_ADOPTION_PLAN_PREFIX
...
def _make_receipt(check, target, result, details, seq, identity=None):
    """Build one current-contract adoption receipt with its stable Gate ID.
    ...
    receipt = kblib.make_receipt(
        ...
        receipt_type_id=
            adoption_lineage_contract.STANDARDS_ADOPTION_RECEIPT_TYPE_ID,
        identity=identity)
...
def _plan_identity(plan):
    """Return the non-version Queue identity this transaction leaves behind.

    The transaction binds the version transition explicitly through
    ``upstream_revision_id_before`` and ``upstream_revision_id_after``; it
    never writes a third, ambiguous bare version field.
    """
    return {
...
def _after_profile_evidence(root, plan, *, expected=None, phase):
    ...
    evidence, errors = queue_runtime.profile_load_evidence(root, manifest)
    if errors:
        raise ValueError(
...
def _non_adoption_projection(name, document):
    ...
    if name == "coverage":
        value.pop("upstream_revision_id", None)
        ...
        if isinstance(contract, dict):
            for field in (
                    "contract_version", "upstream_revision_id",
                    "selected_profile_manifest") + LOAD_FIELDS:
                contract.pop(field, None)
    elif name == "standards":
        for field in (
                "state_revision", "upstream_revision_id", "status",
...
def _new_receipt(phase, result, plan, transaction_id, plan_path, plan_sha,
    ...
    receipt = _make_receipt(
        ...
        "%s Standards adoption %s -> %s" % (
            phase, plan["upstream_revision_id_before"],
    ...
    receipt.update({
        ...
        "before_contract_sha256": queue_runtime.contract_sha256(
            before["progress"]),
        "after_contract_sha256": queue_runtime.contract_sha256(
            after["progress"]),
...
def _prepare_result(root, plan_relative, upstream_root, upstream_ref):
    ...
    if plan.get("upstream_revision_id_after") != resolved_revision:
        raise ValueError(
            "upstream Git ref resolves to %s but plan "
            "upstream_revision_id_after is %s" %
            (resolved_revision, plan.get("upstream_revision_id_after")))
    ...
    plan_errors = queue_runtime.standards_adoption_plan_errors(
    ...
    expected_identity = {
        ...
        "upstream_revision_id": plan["upstream_revision_id_before"],
        "selected_profile_manifest":
            plan["selected_profile_manifest_before"],
    ...
    if (standards_before["upstream_revision_id"] !=
            plan["upstream_revision_id_before"] or
    ...
    after["coverage"]["upstream_revision_id"] = plan["upstream_revision_id_after"]
    after["coverage"]["selected_profile_manifest"] = \
        plan["selected_profile_manifest_after"]
    after["queue"]["upstream_revision_id"] = plan["upstream_revision_id_after"]
    ...
    after_contract["upstream_revision_id"] = plan["upstream_revision_id_after"]
    ...
    gate_stub = kblib.make_receipt(
        queue_runtime.TOOL, queue_runtime.TOOL_VERSION, "required_queue",
        queue_runtime.QUEUE_PATH, "pass",
        "post-adoption required Queue consistency", 90,
        receipt_type_id=queue_check_receipt.RECEIPT_TYPE_ID)
    ...
    synthetic = {
        ...
        "remaining": sum(
            ...
            item.get("state") not in queue_runtime.TERMINAL_STATES),
    }
    ...
    overrides = {
        queue_runtime.COVERAGE_PATH: (coverage_text, after["coverage"]),
        queue_runtime.QUEUE_PATH: (queue_text, after["queue"]),
        queue_runtime.PROGRESS_PATH: (progress_text, after["progress"]),
    }
    final = runtime_validation.validate_runtime(
        root, state_overrides=overrides,
        active_standards_state_override=standards_text,
        extra_receipts=[gate, commit],
        pending_active_standards_receipt_id=commit["receipt_id"])
    if final["errors"]:
...
def _require_canonical_commit_readback(prepared):
    """Re-read this transaction's exact commit from its sole history owner."""
    catalog, errors = adoption_lineage_contract.load_adoption_history(
        prepared["root"])
    if errors:
        raise ValueError(
            "canonical adoption history read-back failed: %s" %
            "; ".join(errors))
    expected = prepared["commit"]
    receipt_id = expected["receipt_id"]
    entry = catalog.get(receipt_id)
    if entry is None:
        raise ValueError(
            "canonical adoption history is missing committed Receipt %s" %
            receipt_id)
    relative, observed = entry
    if relative != adoption_lineage_contract.ADOPTION_RECEIPT_PATH:
        raise ValueError(
            "committed Receipt %s is outside canonical adoption history" %
            receipt_id)
    if observed != expected:
        raise ValueError(
            "canonical adoption history Receipt %s differs from the exact "
            "transaction commit" % receipt_id)
...
def _lock_operation(prepared, receipt_path, abort_id):
    return {
        ...
        "before_queue_sha256": prepared["before_sha"]["queue"],
        "planned_after_queue_sha256": prepared["after_sha"]["queue"],
...
def _commit_transaction(prepared, receipt_path):
    ...
    with kblib.runtime_write_lock(
        ...
        try:
            ...
            resulting = runtime_validation.validate_runtime(prepared["root"])
            if resulting["errors"]:
                raise ValueError(
                    "canonical post-adoption runtime read-back failed: %s" %
                    "; ".join(resulting["errors"]))
            _require_canonical_commit_readback(prepared)
        except Exception as exc:
...
def _run(args):
    ...
    print("Standards adoption %s: %s -> %s; queue_revision %s -> %s" % (
        plan["adoption_id"], plan["upstream_revision_id_before"],
        plan["upstream_revision_id_after"], plan["queue_revision_before"],

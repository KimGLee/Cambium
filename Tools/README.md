...
Terminal projection joins that same evidence observation. Profile admission owns the shared rebinding of identical Profile inputs within it, checking them on first use and again before the observation returns successfully. Gate-specific inputs still receive their own checks; independently called composers and new writer/read-back boundaries do not inherit an earlier currency result.
...
Every Runner CLI-contract query uses the target compiler's fresh full check and rechecks component discovery/bytes, projection and environment before returning. Python parser declarations may read inputs beyond component hashes, so a previous query's success is not reused. Within each compilation, the entrypoint owner shares its source descriptors with parameter capture and source analysis. Execution first re-derives and matches the caller's complete action identity, then consumes that invocation's exact audit step without a second stage selection. No step survives a producer dispatch or becomes a caller-supplied authorization. Independent producer admission, locked currentness and after-image read-back remain.

The MCP stdio Host prepares source-checked Python bytecode in a fresh private directory outside the distribution. Children still execute modules and parser defaults in new processes and perform full CLI validation; only source compilation is shared. Source changes invalidate bytecode by content, not timestamps. Adopter-local bytecode and previous sessions' caches are never adopted; automatic bytecode writes remain disabled. Preparation failures use the original source-only path, and normal or exceptional stdio exit releases the directory. This is not a validation cache or a runtime object.

...
`check_queue`, `check_batch_close`, `apply_task_plan`, `compile_queue`, `update_task`,
...
| `project_page_state.py` | K08/07 page-state projector: rewrites the page-frontmatter copies of the Ledger-owned projection fields (`coverage_disposition`, `authoring_status`, `next_batch`) to the Coverage Ledger owner values — update on divergence, remove when the owner value is empty, never add a field the page does not carry; the batch close runs it (or the integrator acts as it) after the close projection so no page keeps a pre-close status or a stale routing to a closed batch; dry run by default, `--page` scopes, `--apply` writes atomically | `python3 Tools/project_page_state.py . --apply` |

...
| `path_capability.py` | Shared descriptor-retained path protocol 1.0.0. It validates the MCP child manifest, reads admitted files without reopening names, materializes admitted trees, opens stable parents for append/replace/transaction publication, advances transaction identity after replacement, propagates descriptors to nested Cambium subprocesses, and acknowledges actual consumption. It owns no policy registry, argument parser, or tool verdict | imported by `kblib.py`; no command-line entry point |
| `kblib.py` | Shared library 1.11.0 and sole restricted-YAML parser owner. Duplicate keys and unsupported constructs fail closed; it also provides deterministic YAML rendering, repository-contained managed paths, a shared Git-managed content-set enumerator, typed snapshots for existing or safely missing repository targets, file and repository-snapshot SHA-256 fingerprints, atomic writes, Markdown helpers, receipts, the mandatory Cambium-child subprocess boundary, and the public tool-facing facade over `path_capability.py` | imported by the scripts above |

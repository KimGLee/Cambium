...
`record_batch_judgment`, `record_gate_attestation`, `record_gate_result`, `apply_metadata_transition`,
`check_residual_content`, `scaffold_profile`, `profile_onboarding_status`,
`profile_contract`, `profile_admission`, `metadata_execution_contract`,
`metadata_gate_runtime`, `metadata_property_state`, `stamp_cards`, `run_gates`,
...
`contract_exception_policy`,
`coverage_delta`, `freshness_engine`, `maintenance_candidates`,
`path_capability`, and `kblib`.
...
## Card-first activation delivery

`check_queue.py --require-ready BATCH --json` compiles the frozen Task
Contract into one `card-first-readback-v1` Bundle. The JSON receipt contains
the content-addressed manifest; the JSON tool result additionally carries
transient exact R01/task-Card text and startup read-backs for Host injection.
Durable JSONL never duplicates those bytes. `update_queue.py` recompiles that
Bundle before `queued -> open` and copies its hashes and delivery identity into
the transition receipt. Through `mcp_server.py`, both operations share one
`CAMBIUM_EXECUTION_CONTEXT_ID`; another MCP session cannot consume the first
session's machine-delivered admission. Direct CLI use receives the same
payload with `delivery_assurance: degraded` because no model context can be
identified. Conditional source delivery uses
`check_queue.py --deliver-readback BATCH --readback-rule RULE --json`; the
returned Addendum is bound to the parent Bundle and refuses source drift.

## Tool inventory

| Script | Purpose | Typical invocation |
|---|---|---|
| `run_gates.py` | Adopter verification sweep 1.0.0 (K00/12 Verification Run and Process Contract). The set it runs is DERIVED, never listed: every Stable Gate ID Registry row whose producer is a named deterministic tool and whose Lifecycle is `not-batch-scoped`. Preflights the registry/producer agreement guard and compiled-artifact freshness (`compose_vocab --check`, `compose_page_contract --check`, `compile_cli_contract --check`, `render_interface_projection --check` -- the last two bind their projection target, boundary hash and tool set, so an interface artifact carried in from another repository is refused here rather than serving); reports `manual-attestation` rows as human-recorded (producing the `stamp_cards --check` input for `runtime-card-synchronization`) and transaction writers (`adopt_standards`, `record_corpus_acceptance`) as never-swept; a registry row it cannot classify fails the run closed so the registry cannot outgrow its executor silently. One `check_vocab` run serves both of that producer's gates. In an adopter runtime (selected profile outside `profiles/examples/`) it also reports a candidate per `distribution-boundary.yaml` tree present in the working tree (owner K00/03 Distribution Boundary). Exit codes follow the shared process contract: `1` = at least one failure, `2` = no failure but holds each of which a person must read, `0` = clean; `--list` prints the derivation without running; `--profile` overrides the live runtime's selected manifest; `--exclude` passes through to scanners that accept it | `python3 Tools/run_gates.py . --exclude _to_delete` |
| `check_links.py` | Wiki link checker 1.6.0 over the Git-managed content set; missing / ambiguous / heading verification (K09/03, K09/05); `--scope` accepts a directory or a single page; the effective scan set is checked after exclusions, and zero files fail for both scoped and whole-root runs; an exact full-path link into an excluded area resolves as `excluded_target` before any active basename fallback | `python3 Tools/check_links.py . --receipts Tools/receipts/links.jsonl` |
...
| `tool_availability.py` | Shared library and sole reader of `distribution-boundary.yaml`, not a command. It answers one question for the interface compile chain: which tool modules a declared projection target is allowed to be missing. `source-distribution` may be missing nothing, because the distribution owns every tool it declares; `carried-runtime` may be missing exactly the `Tools/*.py` entries the boundary names, and nothing else. The target is supplied by the caller and recorded in what the caller produces -- never inferred from which files happen to be present, because a half-checked-out distribution would otherwise identify itself as an adopter runtime and excuse the very absence that needs reporting. `partition()` splits a declared tool list into included, boundary-excluded, and unregistered-absent, and the third is what keeps a carried tool that failed to arrive from passing as an excluded one | imported by the scripts above; no command-line entry point |
| `profile_admission.py` | Shared consumer adapter, not a command. It selects the explicit or canonical-state-approved Profile, performs exactly one complete `profile-load`, exposes the authorized immutable Profile snapshot and typed slot paths/text, and rechecks Profile-tree, root-input, and active-selection currency before a consumer emits a pass result or writes an artifact. Profile-dependent tools use this view instead of reopening the manifest or slot files under a later revision | imported by the Profile-dependent scripts above; no command-line entry point |
| `metadata_execution_contract.py` | Closed metadata-execution authority compiler and loader 1.0.0. It compiles Kernel field/transition rules with the installed operation capability registry, requires exact rule/operation coverage, and requires every writer/consumer/producer/receipt-schema capability to name canonical `Tools/*.py` implementation paths. Stable no-follow hashes of the complete implementation set enter `compiled/metadata-execution-contract.json`; the loader refuses source, implementation-byte, or artifact drift. Its output is fixed to the registered compiled artifact, so the compiler cannot overwrite one of its own integrity inputs. Its pure path API lets `profile-load` freeze those files inside the same canonical root-input view; the successful producer retains that exact compiled object as the Profile-load-covered derived runtime authority, so admitted writers do not reopen a second observation | `python3 Tools/metadata_execution_contract.py --root . --check` |
| `record_gate_attestation.py` | Manual Extension Gate evidence producer 1.0.0. It resolves one exact Gate only from the current snapshot-bound typed Profile contract, requires the Gate's registered pass-authority role and completion enum, and emits a `manual-gate-attestation-v1` receipt binding the page's projection-neutral semantic fingerprint, exact page bytes, selected Profile manifest/tree/typed-contract/root-input fingerprints, active K00 revision, and metadata execution contract. It writes no state or page; dry-run is default | `python3 Tools/record_gate_attestation.py . --gate-id P:<profile>:<gate> --page <path.md> --value <registered-value> --actor-role <pass-authority-role> --statement "Bounded judgment" --apply` |
| `record_batch_judgment.py` | Sole per-target Batch Review judgment scribe 1.0.0 (K12/14): answers one frozen Batch Review Requirement record for one open batch — validates the target against the activation-frozen expansion, the reviewer against the Profile's registered pass-authority role, and binds the judgment to the batch's current activation receipt, the target's semantic content fingerprint, and the authorized Profile contract fingerprint, so reopening the batch, editing the page, or revising the Profile invalidates the evidence. Rebuilds every binding under the runtime write lock immediately before publication and fails closed on any drift. Changes no page, Ledger, or Queue state; `update_queue.py` consumes the exact record set at `open -> merge-ready` through the batch-review wrapper. | `python3 Tools/record_batch_judgment.py . --batch <id> --judgment-item <item> --target <page> --reviewer-role <role> --statement "..." --apply` |
| `record_gate_result.py` | Deterministic Extension Gate evidence adapter 1.0.0. It resolves one exact typed Gate and the Gate's exact Registered Scan from the same authorized Profile view, stages the no-follow-hashed scanner, its closed Python runtime inputs, and the Profile-snapshot configuration as immutable execution inputs, then runs the compiled shell-free command with adapter-owned temporary receipt output. It accepts only a clean pass set and exact final scan/input identities, rejects repository or live-tool drift, and wraps the source summary into `deterministic-gate-result-v1`. The result binds the sole Gate completion value, exact semantic page/Profile/K00/metadata identities, source receipt/set/command/tool/config/runtime/execution-input hashes, and current repository snapshot. It writes no owner/page state; apply appends only the bound Gate result under the shared writer lock | `python3 Tools/record_gate_result.py . --gate-id P:<profile>:<gate> --page <path.md> --apply` |
| `apply_metadata_transition.py` | Sole `metadata-transition-integrator-v1` consumer 1.0.0 for typed Profile Extension Gates. It accepts only a matching pass receipt in the current filtered catalog, derives the enum field/value and generic Coverage owner rule from the authorized Gate rather than caller-supplied schema, writes `Coverage.pages[].property_state`, and projects the page through the common page-state composite transaction. Profile, K00, metadata-contract, Coverage and page identities are CAS-bound under the shared writer lock; Coverage and the exact page before-image roll back together before receipt publication, while uncertain append/cleanup retains recovery evidence. Dry-run is default and apply is Integrator-only | `python3 Tools/apply_metadata_transition.py . --gate-id P:<profile>:<gate> --page <path.md> --value <registered-value> --gate-receipt <receipt-id> --actor-role integrator --expected-coverage-sha256 sha256:<hex> --expected-page-sha256 sha256:<hex> --apply` |
| `metadata_gate_runtime.py` | Shared closed adapter for typed Extension Gate execution. It consumes the one authorized Profile view, resolves one exact Gate, verifies installed producer/schema/consumer capabilities, derives a single evidence-backed enum projection rule, constructs the common receipt binding, and validates evidence against a caller-supplied current catalog. It admits no runtime of its own: the caller passes the admitted runtime and the authority frozen from that same admission, and this module refuses a pair assembled from two. It performs no writes and accepts no policy callback | imported by the three Extension Gate CLIs above and, for persisted-owner validation only, by `check_queue.py`; no command-line entry point |
...
| `compile_cli_contract.py` | Persistent CLI invocation-contract compiler 1.1.0: derives `Tools/compiled/cli-contract.yaml` from the `argparse` declaration each `Tools/*.py` CLI builds for itself and the closed `Tools/agent-interface-policy.yaml`. Argparse remains the invocation source; the policy adds the separate capability facts argparse cannot express: MCP versus CLI-only exposure, the workspace-root argument, typed path arguments, contained/exact/namespace restrictions, snapshot/append/replace/transaction consumption, operation-mode activation predicates, and any explicit external-write effect. Every argparse argument must be classified exactly once as workspace, filesystem path, or ordinary value, so a new tool or argument fails compilation instead of inheriting an ambient transport capability. Each tool is imported with `parse_args` patched to raise the instant its parser is complete, so no tool behaviour runs. The registered output is fixed to `Tools/compiled/cli-contract.yaml`; `--check` exits 2 when it is stale or hand-edited, 1 only when the evidence itself is unreliable | `python3 Tools/compile_cli_contract.py . --check` |
| `render_interface_projection.py` | Agent-facing form projection 1.1.0: projects `Tools/compiled/cli-contract.yaml` into the interface shapes an agent runtime actually reads. The `mcp` form includes only operations whose closed policy declares `exposure: mcp`, carries each tool's workspace binding and each path argument's access, consumption mode, operation-mode activation predicate, and contained/exact/namespace constraint, and writes only the registered `Tools/compiled/mcp-tools.json` artifact. CLI-only compilers and host installers never enter the table. Argparse names, descriptions, actions, types, choices, and required fields remain projected rather than restated; every emitted field is bound in `FIELD_SOURCES`, and an unbound field fails the run. The artifact carries the sha256 of the contract bytes and its source manifest hash. `--check` exits 2 when an artifact is stale or hand-edited, and 1 when the evidence is unreliable | `python3 Tools/render_interface_projection.py . --check` |
| `render_host_configs.py` | MCP server registration and corpus binding 1.1.0: renders the one server definition body this tool declares (`command`, `args`, `cwd`, `env`, and the dsh-only connection-resilience superset) into the configuration file each supported host actually reads. Registration -- where the server is and how it starts -- is once per machine; binding -- which corpus this run governs, carried as `CAMBIUM_WORKSPACE_ROOT` -- is once per corpus. `HOSTS` is a registry with one builder and one output file per host: Claude Code (`<corpus>/.mcp.json`), Kimi Code (`<corpus>/.kimi-code/mcp.json`), Codex (`<corpus>/.codex/config.toml`, loaded only for a trusted project), dsh's per-corpus `.env` (binding only) and dsh's `$DSH_HOME/profiles/<name>/` rows (registration only). The five products are templates rendered under `Tools/compiled/host-configs/` for an adopter's corpus repository; this distribution registers no MCP server with itself and writes none of these files at its own root. Every product carries `CAMBIUM_INTERFACE_SOURCE_HASH`, the sha256 of the `compiled/mcp-tools.json` bytes it was rendered against, so one upstream change makes all five stale at once. Every rendered field is bound in the tool's own `FIELD_SOURCES` table -- the server name included, because the name is spelled into those paths -- and a field no source covers fails the run; `--sources` prints that table. `--distribution-root` and `--workspace-root` substitute the two placeholders for an onboarding flow writing a bound copy. It is machine-generated and must not be hand-edited, and it registers no K00/12 Gate ID for the same reason its upstream does not. `--check` exits 2 when a product is stale or hand-edited, and 1 when the evidence is unreliable | `python3 Tools/render_host_configs.py . --check` |
| `mcp_server.py` | Stdio MCP server 1.1.0: the layer-3 entry point named by every generated host configuration. It serves the pinned `compiled/mcp-tools.json`, runs each operation as `Tools/<tool>.py <args> --json` in a subprocess, and carries exit `0`/`1`/`2` as clean/failed-or-unreliable/HOLD without reinterpreting tool judgment. `CAMBIUM_WORKSPACE_ROOT` is the session binding: initialization opens and pins its canonical directory object and filesystem identity, path inspection walks from that descriptor, and the descriptor becomes the child cwd. Every active typed path is retained as the admitted file and/or parent directory descriptor according to its snapshot/append/replace/transaction mode, passed in a canonical manifest, and consumed through the shared path-capability API; nested Cambium subprocesses retain the same authority, and a successful or held call may not ignore any active path capability. Replaced or retargeted workspaces and path parents, missing or alternate roots, detached CLI modes, absolute/`..` paths, symlink aliases, multiply-linked regular files, alternate exact artifacts, and wrong namespace suffixes cannot redirect the child. Enum membership, state authorization, evidence sufficiency, and tool verdicts remain with the tool. The server imports no Cambium judgment module, and a missing, stale, malformed, capability-incomplete projection, or unsupported no-follow descriptor platform fails initialization | started by a host: `python3 Tools/mcp_server.py` |
| `check_freshness.py` | Freshness check 2.1.0 and CLI adapter for `freshness_engine.py`: emits the engine's complete candidate set rather than an overdue-only projection. A temporally valid selected baseline plus resolved volatility is required before `review_by` is computed; content modified after its current review, overdue, pending-first-verification, invalid or future explicit event, invalid or unresolved volatility, and unparseable-frontmatter outcomes are candidates. `last_content_modified` is an Integrator event projection: when it is later than `last_reviewed`, old review evidence cannot authorize the current content. Only an absent or blank `last_verified` permits fallback to `last_reviewed`, and only an absent or blank explicit `volatility` permits fallback to the Profile default; an explicit invalid value remains a candidate. Every active in-scope page reaches a closed outcome, so an unresolved page cannot be skipped inside an otherwise passing run; an in-scope file with unparseable frontmatter is conservatively a candidate. `stable` removes recurring due dates, not the need for an initial completed review or verification; the diagnostic file-modification fallback is normalized to a UTC calendar date. `--defaults` accepts a flat mapping or the admitted current `Tools/vocab.yaml` / Profile vocabulary extensions, with their exact snapshot fingerprints. Every complete scan emits one typed summary binding the canonical scope, exact input snapshot, classification counts, and sorted candidate-set fingerprint. Exit 0 means the complete nonempty scope was classified and no candidate exists; candidate findings exit 2 | `python3 Tools/check_freshness.py . --as-of 2026-07-21 --defaults profiles/<your-profile-id>/vocabulary-extensions.yaml --exclude Cards --receipts Tools/receipts/fresh.jsonl` |
| `freshness_engine.py` | Shared pure freshness classifier, not a command: receives already-loaded page facts, the effective volatility policy, and one `as_of` boundary; validates completed-event causality before volatility exemptions, classifies every active in-scope page into a closed outcome, and returns typed findings plus deterministic summary counts. It performs no traversal, Profile discovery, writes, receipts, CLI parsing, candidate fusion, budget ordering, or status mutation | imported by `check_freshness.py`; no command-line entry point |
...
| `contract_exception_policy.py` | Pure contract-exception policy. It owns the closed registry of policies a K13/02 exception may except, each entry's kernel owner and limit domain, the policy families and comparison protocol versions, the Priority-quota and Coverage effective-policy resolvers together with the canonical fingerprint an exception binds to, and the K00/07 joint effective-ceiling fold over standing quotas plus granted exceptions. It reads no state, writes nothing, and reaches no verdict; the exact share comparison stays with `kblib.quota_share_within_limit` | imported by profile, Queue, close, Gate-runner, and contract-amendment consumers; no command-line entry point |
| `path_capability.py` | Shared descriptor-retained path protocol 1.1.0. It validates the MCP child manifest, reads admitted files without reopening names, materializes admitted trees, opens stable parents for append/replace/transaction publication, advances the exact capability to a newly replaced transaction inode or a receipt inode first created by append, verifies that an observed append target remains the unique admitted name, propagates advanced descriptors and manifests to nested Cambium subprocesses, and acknowledges actual consumption. It owns no policy registry, argument parser, or tool verdict | imported by `kblib.py`; no command-line entry point |
| `kblib.py` | Shared library 1.12.0 and sole restricted-YAML parser owner. Duplicate keys and unsupported constructs fail closed; it also provides deterministic YAML rendering, repository-contained managed paths, a shared Git-managed content-set enumerator, typed snapshots for existing or safely missing repository targets, file and repository-snapshot SHA-256 fingerprints, atomic writes, Markdown helpers, receipts with descriptor-bound first-publication proof, the mandatory Cambium-child subprocess boundary, and the public tool-facing facade over `path_capability.py` | imported by the scripts above |
...
1. At the start of every task, test whether `.cambium/state/` exists. If it does, run
...
2. Only when `.cambium/state/` is absent, and only for a selected persistent,
   ...
   definition. Preserve any governance state and adoption history already in
   `.cambium/`. Declare exactly one
...
5. Before activation, resume, or completion entry, compare the canonical
   adopter Standards/Profile identity with the Task Contract and all three task state
...
valid but stale Card layer exits 2 in `--check` mode. `source_hash` is the
observed digest and is advanced by ordinary stamping; `compiled_source_hash`
is advanced only by `--acknowledge-compiled` after semantic regeneration or
review. Only exact structure, equal current hashes, and agreement with the
canonical adopter `standards_version` (or the explicit public-distribution
template) exits 0.
...
names cannot be reached by any routed task. Within each paired route, every such
boundary leaf must also appear exactly once in the Card's disposition: in
`source_files` when its semantics are compiled into the Card, or in
`readback_sources` when the Card intentionally delegates it to source reading.
Missing, overlapping, and extra dispositions fail closed. A missing or
...
- **Persistent task / multi-batch control** = `check_queue.py --resume-status`
  at every restart or new-Agent entry when `.cambium/state/` exists, then the Queue
...
- **Active-task Standards adoption** = K12/10 validates one restricted-YAML
  changed-predicate plan; `adopt_standards.py` is the only writer of the
  canonical adopter state, three synchronized task identities, and Progress
  Contract/load set. R09 creates
...
- **Governance** = `stamp_cards.py . --check`, `check_moc.py`, and the
  `check_profile.py` 2.0.0 `profile-load` Gate against each filled profile a
  deployment selects.
...
- `standards_adoption_plan.template.yaml` -- the closed restricted-YAML
  ...
  K12/10; transaction owner: K13/15); it binds approved K00/03 rule bytes, the
  exact canonical adopter-state before-image, deterministic after
  Kernel/Profile snapshots, Task/Contract identity, Queue revisions, all four
  state SHA-256 values, the changed predicates, the
...
- `profile_adoption_plan.template.yaml` -- the closed restricted-YAML plan
  ...
  canonical state after values, append-only receipt detail, an empty
  `changed_predicates` list (a nonempty list belongs to
  `adopt_standards.py`), and the compare-and-swap fingerprints: exact
  current K00/03 rule bytes plus the absent/current state and candidate Profile's `profile-load` tree
...
- `key: value` scalars: strings (optionally quoted), integers, floats,
  booleans, null, the inline empty list `[]`, the exact inline empty mapping
  `{}`, and simple inline lists `[a, b]`;
...
`>`), non-empty flow maps such as `{key: value}`, tags, multi-document streams,
tab indentation.
...
`compile_cli_contract.py` from both the `argparse` declaration every
`Tools/*.py` CLI builds for itself and the closed
`agent-interface-policy.yaml`. It is the machine-readable statement of how
these tools are called and which of those calls an agent transport may expose:
per argument the `option_strings` (empty exactly for a positional), `dest`,
`required`, evaluated `default`, `choices`, `nargs`, `action`, `type` name and
`help`, plus each mutually exclusive group, the receipt extension fields, and
the workspace/path capability classification. Each typed path additionally
carries one closed consumption mode: read=`snapshot`, ordinary
write=`replace`, read-write=`transaction`, with append-only registers
explicitly narrowed to `append`. A path override may name a same-tool boolean
mode that makes an otherwise effective default inactive; this is used only
when argparse exposes a path that the selected mode provably does not consume.
`choices` is recorded as a
canonically ordered set, because several tools build it from a Python set whose
iteration order is not stable between processes. Tool rows and argparse
arguments must close exactly against the policy; a new unclassified one fails
compilation. Do not edit it by hand; regenerate with
`python3 Tools/compile_cli_contract.py .` and verify with
`python3 Tools/compile_cli_contract.py . --check`, which `make check` runs. It
carries no Gate ID: it depends on no selected profile, and `run_gates` cannot
start before a profile is selected.
...
MCP-exposed subset of the same interface statement in the shape a Model
Context Protocol runtime reads: per tool a `name`, the tool's own argparse
`description`, a workspace capability, and an `inputSchema` whose properties
are keyed by `dest` and carry typed path capabilities where applicable. The
mapping is mechanical -- an
...
`argparse` parser and no `main()`, so `compile_cli_contract` does not see it and
it never appears in the tool list it serves. The current compiled contract
closes over 46 CLI tools; the MCP projection contains the 40 whose policy rows
declare MCP exposure and excludes six build/install operations. Do not edit
these products by hand; regenerate with
`python3 Tools/render_host_configs.py .` and verify with
`python3 Tools/render_host_configs.py . --check`, which `make check` runs
...
`vocabulary-extensions.yaml` of the Profile selected by the canonical adopter
Standards state.
...
Profile of its own. Every run reads the adopter Standards state; an
argument-free run derives the
...
because the public distribution intentionally has no adopter Standards state
and selects no Profile. What is
...
This command succeeds only after initial adoption selects that Profile in the
canonical state. After that,
...
Authoritative definitions remain in the Kernel leaves. A Card's `source_files`
are the direct inputs compiled into its guidance; `readback_sources` are
explicitly delegated to the paired Read Set. After revising an owner standard,
stamp observed hashes, regenerate or review affected Cards, explicitly
acknowledge the compiled digest, and run `--check`. Never edit only a Card or
cite it as standards text when adjudicating a conflict. A profile may add a
...
This layer refuses calls for which no argv can be formed at all: an undeclared
argument name, a value whose type cannot be rendered onto argv, and a missing
positional that would silently rebind a later positional's value onto an
earlier parameter. Before forming argv it also enforces the compiled transport
capabilities: the workspace argument must equal the session binding, and every
effective active path value -- caller-supplied or an omitted argparse default
-- must stay inside its contained, exact, or namespace envelope without a
symlink or hard-link alias. The transport then retains the admitted file and/or
parent directory descriptors, passes them with a canonical per-call manifest,
and requires every active capability to be acknowledged by a
descriptor-backed consumer before a clean or held result is returned. A mode
that does not consume an otherwise effective path, or only consumes it when an
apply-like flag is present, must declare that activation predicate in the
compiled interface; it cannot silently leave a write path unused. Distinct
active arguments may not alias one path under the same consumption mode, so
one read or write cannot acknowledge a different semantic argument. It does not
check `enum` membership, general required-argument presence, state
authorization, or evidence sufficiency; those judgments remain with the tool.

### The binding

`CAMBIUM_WORKSPACE_ROOT` names the corpus a session governs and is read from the
environment only. There is no working-directory fallback: cwd inheritance is
undocumented best-effort in all four hosts, and a binding may not rest on one.
Unset, blank, relative, un-substituted, or naming something that is not a
directory refuses `initialize` with a message naming the variable the host has
to set. Initialization opens and freezes the root's canonical directory object
and filesystem identity; every call revalidates the configured name, while the
open descriptor itself is carried through path inspection and into the child
working directory. Renaming, replacing, or retargeting that path therefore
cannot transfer either a later or an in-flight call to another corpus. The
child receives `.` for the policy-declared workspace argument so it resolves
from that pinned directory object. `path_capability.py` validates the inherited
manifest and owns the retained-object protocol; `kblib.py` exposes the shared
snapshot, append, replace, and transaction I/O facade used by tools. Nested
Cambium subprocesses pass the same descriptors and acknowledgement channel
instead of silently reopening argv names. An operating system without
`O_DIRECTORY`, `O_NOFOLLOW`, and descriptor-relative child execution fails
server initialization; no degraded concurrent-path assurance is advertised.

This guarantee covers the active typed paths in the public MCP call surface.
It is not protected whole-workspace execution: fixed or derived internal paths
that are not projected arguments, simultaneous replacement of repository code
and evidence, and non-filesystem effects remain outside this transport
contract. Those wider guarantees belong to an isolated workspace adapter or an

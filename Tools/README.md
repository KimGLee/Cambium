...
| `check_freshness.py` | Freshness check 1.4.0: computes review_by from volatility and last_verified (fallback: last_reviewed, then file modification time per K08/05, flagged pending first verification); a selected review/verification baseline later than `--as-of` is a candidate and is never used as freshness evidence; `--defaults` accepts a flat mapping or `Tools/vocab.yaml` / a profile's `vocabulary-extensions.yaml` (their `volatility_defaults`); canonical `Tools/vocab.yaml` is consumed only through the current admitted Profile and immutable artifact bytes, while explicit flat defaults remain standalone inputs; an all-skip run reports NOTHING CHECKED as a candidate, not a pass | `python3 Tools/check_freshness.py . --as-of 2026-07-21 --defaults profiles/<your-profile-id>/vocabulary-extensions.yaml --exclude Cards --receipts Tools/receipts/fresh.jsonl` |

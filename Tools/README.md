...
| `mcp_server.py` | Stdio MCP server 1.0.0: the layer-3 entry point named by every generated host configuration. It serves the pinned `compiled/mcp-tools.json`, runs each operation as `Tools/<tool>.py <args> --json` in a subprocess, and carries exit `0`/`1`/`2` as clean/failed-or-unreliable/HOLD without reinterpreting tool judgment. `CAMBIUM_WORKSPACE_ROOT` is the session binding: initialization opens and pins its canonical directory object and filesystem identity, path inspection walks from that descriptor, and the descriptor becomes the child cwd so a pathname replacement cannot transfer an in-flight call. Every projected call must supply its policy-declared workspace argument, that argument must resolve exactly to the frozen root and is normalized to `.` for child execution, and every effective typed path -- including an omitted argparse default -- must use canonical repository-relative spelling inside its contained, exact, or namespace envelope. A replaced or retargeted workspace, missing or alternate roots, detached CLI modes, absolute/`..` paths, symlink aliases, multiply-linked regular files, alternate exact artifacts, and wrong namespace suffixes are protocol refusals before child execution; enum membership, state authorization, evidence sufficiency, and tool verdicts remain with the tool. The server imports no Cambium judgment module, and a missing, stale, malformed, or capability-incomplete projection fails initialization | started by a host: `python3 Tools/mcp_server.py` |
...
the workspace/path capability classification. `choices` is recorded as a
...
effective path value -- caller-supplied or an omitted argparse default -- must
stay inside its contained, exact, or namespace envelope without a symlink or
hard-link alias. It does not check `enum` membership, general required-argument
presence, state authorization, or evidence sufficiency; those judgments remain
with the tool.
...
from that pinned directory object; ordinary caller-supplied path arguments are
not rewritten, while omitted path options are checked against their compiled
argparse defaults before the child can consume them.

This is not a claim of adversarial concurrent namespace exclusion. A separate
process that can rewrite arbitrary workspace children after inspection is
already inside the local evidence trust boundary described above. Closing that
wider case requires protected execution or migrating every tool's reads,
creates, and replacements to one stable path-object API; a transport-only
pathname recheck cannot close it.

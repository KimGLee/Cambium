...
current member identities, meanings, and order. This module owns only strict
loading, mechanical validation, the evidence-field projection, and the
...
_MEMBER_FIELDS = {"member_id", "meaning"}
...
def validate_batch_close_closed_list(document):
    ...
    for index, member in enumerate(members):
        ...
        if not isinstance(member, dict) or set(member) != _MEMBER_FIELDS:
            raise ValueError("%s fields must be exactly member_id, meaning" %
                             label)

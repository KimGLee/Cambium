...
_HOST_TRANSPORT = agent_interface_policy.shared_host_transport(REPO_ROOT)
HOST_TRANSPORT_ID = _HOST_TRANSPORT["transport_id"]
...
UPSTREAM_ARTIFACT = agent_interface_contract.PROJECTION_ARTIFACT_KIND
UPSTREAM_FORM = agent_interface_contract.MCP_FORM
UPSTREAM_SCHEMA_VERSION = agent_interface_contract.PROJECTION_SCHEMA_VERSION
...
SERVER_NAME = _HOST_TRANSPORT["server_name"]
...
# Why the shared transport comes from source policy rather than a compiled
...
# declared inputs" that owns no independent rule. The shared server identity,
# command, entry point, protocol and mode are declared once in
# `agent-interface-policy.yaml`; this module only projects that row into each
# Host format. Host-specific resilience values remain implementation constants.
#
# Every product below is built from the loaded transport row and every rendered
# field is admitted through `FIELD_SOURCES`.
...
DSH_TRANSPORT = _HOST_TRANSPORT["mode"]
...
SERVER_COMMAND = _HOST_TRANSPORT["command"]
SERVER_ENTRY_POINT = _HOST_TRANSPORT["path"]
...
def main(argv=None):
    ...
    if unsafe_component is not None:
        return fail(
            ...
            "%s" % repository_relative_spelling(root, unsafe_component))
    ...
    context = {
        "source": repository_relative_spelling(root, projection_path),
    ...
    try:
        recheck = file_bytes_sha256(projection_path)
    except OSError as exc:
    ...
    if args.check:
        ...
        if intruders:
            return fail(
                ...
                % (SKILL_MANIFEST,
                   repository_relative_spelling(root, output_dir),
                   ", ".join(repository_relative_spelling(root, path)

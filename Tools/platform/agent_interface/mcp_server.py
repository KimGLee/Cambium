...
It imports no check, applier, or judgment module. Its repository imports are
limited to runtime path spellings, repository-root mechanics, and the pure
argv renderer shared with the Task Runtime Runner. This is the executable form
of "layer 3 only carries": the transport can enforce its compiled interface
without gaining access to a governance judgment. `Tools/tests/test_mcp_server.py`
asserts that narrow import boundary statically so it survives future edits.
The remaining imports are from the standard library; sha256 and canonical
`json.dumps` stay local so no judgment seam can be crossed by accident.
...
# Bootstrap before the repository-local imports below.  A carried MCP
...
# This minimal bootstrap is intentionally duplicated: importing a shared
# repository helper here could consume the local bytecode it exists to avoid.
def _external_pycache_prefix():
    current = os.path.realpath(os.path.abspath(__file__))
    while os.path.basename(current) != "Tools":
        parent = os.path.dirname(current)
        if parent == current:
            raise RuntimeError("MCP implementation is outside Tools")
        current = parent
    repository_root = os.path.dirname(current)
...
# same way. `agent-interface-policy.yaml` is the machine declaration; this
# narrow transport keeps the runtime label local and a boundary test binds it
# back to that declaration without importing policy machinery here.
SERVER_NAME = "cambium"
...
DISTRIBUTION_ROOT = repository_source_root(__file__)
...
EXECUTION_CONTEXT_ENV = agent_interface_contract.EXECUTION_CONTEXT_ENV
...
SOURCE_HASH_ENV = agent_interface_contract.INTERFACE_SOURCE_HASH_ENV
PROJECTION_PATH_ENV = agent_interface_contract.INTERFACE_PROJECTION_ENV
...
PROJECTION_ARTIFACT = agent_interface_contract.PROJECTION_ARTIFACT_KIND
PROJECTION_FORM = agent_interface_contract.MCP_FORM
PROJECTION_SCHEMA_VERSION = \
    agent_interface_contract.PROJECTION_SCHEMA_VERSION
SOURCE_DISTRIBUTION_TARGET = \
    agent_interface_contract.SOURCE_DISTRIBUTION_TARGET
            ...
            for label, conditions in (
                ...
                if (not isinstance(conditions, list) or
                        ...
                        any(not isinstance(condition, str) or
                            ...
                            "boolean" or
                            cli_argv_renderer.cli_metadata(

...
_CardContractError = card_contract.CardContractError
_load_card_schema = card_contract.load_schema


def _safe_relative(value, label, *, suffix=None):
    try:
        return repository_path_contract.canonical_repository_relative_path(
            value, label, suffix=suffix)
    except ValueError as exc:
        raise _CardContractError(str(exc)) from exc


def _frontmatter(text, label):
    raw = kblib.extract_frontmatter(text or "")
    if raw is None:
        raise _CardContractError("%s has no YAML frontmatter" % label)
    try:
        value = kblib.parse_yaml_subset(raw)
    except (ValueError, kblib.YamlSubsetError) as exc:
        raise _CardContractError("%s frontmatter is invalid: %s" % (label, exc))
    if not isinstance(value, dict):
        raise _CardContractError("%s frontmatter must be a mapping" % label)
    return value
...
def _body(text):
    ...
    if not text.startswith("---\n") or end < 0:
        raise _CardContractError("Card has no fenced frontmatter")
...
def _load_budget(root):
    try:
        ...
        raise _CardContractError("%s is unsafe or invalid: %s" %
                                (CARD_BUDGET_PATH, exc))
    expected = {"schema_version", "max_body_bytes", "max_action_items"}
    if not isinstance(value, dict) or set(value) != expected:
        raise _CardContractError("%s has an invalid closed shape" % CARD_BUDGET_PATH)
    if value.get("schema_version") != 1:
        raise _CardContractError("unsupported Card budget schema_version")
    for field in ("max_body_bytes", "max_action_items"):
        number = value.get(field)
        if isinstance(number, bool) or not isinstance(number, int) or number <= 0:
            raise _CardContractError("%s.%s must be a positive integer" %
...
def source_digest(root, relative_paths):
    ...
    for relative in relative_paths:
        try:
            ...
            raise _CardContractError("source %s is unsafe or unreadable: %s" %
                                    (relative, exc))
...
def replace_frontmatter_scalar(text, field, value):
    ...
    if not text.startswith("---\n") or end < 0:
        raise _CardContractError("missing fenced frontmatter")
    front = text[:end]
    pattern = re.compile(r"(?m)^%s:\s*.*$" % re.escape(field))
    if not pattern.search(front):
        raise _CardContractError("missing frontmatter field %s" % field)
    scalar = str(value)
    if "\n" in scalar or "\r" in scalar or "'" in scalar:
        raise _CardContractError("frontmatter scalar %s is not safely quotable" % field)
    front = pattern.sub("%s: '%s'" % (field, scalar), front, count=1)
    return front + text[end:]


def discover_cards(root):
    """Return the current Card/Read Set machine registry.

    The two canonical declaration sets are validated and returned without a
    compatibility shape or generated-index fallback.  Planning, activation,
    audit, and this projection writer consume this same result.
    """
    root = Path(root).resolve()
    schema = _load_card_schema(root)
    directory = root / schema["directory"]
    if not directory.is_dir() or directory.is_symlink():
        raise _CardContractError("canonical Card directory is missing or unsafe")
    ...
    for path in sorted(directory.glob("*.md")):
        ...
        if (data.get("type") != schema["document_type"] or
            ...
            raise _CardContractError("%s must be a curated Card" % relative)
        route_id = data.get("route_id")
        if (not isinstance(route_id, str) or
                not schema["route_id_re"].fullmatch(route_id)):
            raise _CardContractError("%s has invalid route_id %r" %
                                    (relative, route_id))
        if route_id not in read_sets:
            raise _CardContractError("%s route_id %r has no Read Set declaration" %
                                    (relative, route_id))
        if route_id in records:
            raise _CardContractError("more than one Card declares %s" % route_id)
        if not path.name.startswith(route_id + " "):
            raise _CardContractError("%s filename must start with %s" %
                                    (relative, route_id))
        if data.get("read_set_id") != route_id:
            raise _CardContractError("%s read_set_id must equal route_id" % relative)
        expected_read_set = read_sets[route_id]["path"]
        if data.get("read_set") != expected_read_set:
            raise _CardContractError("%s must bind %s" % (relative, expected_read_set))
        sources = card_contract.string_list(
            data.get("source_files"), "%s source_files" % relative,
            nonempty=True)
        for source in sources:
            _safe_relative(source, "%s source" % relative)
        if expected_read_set not in sources:
            raise _CardContractError("%s source_files must include its Read Set" % relative)
        ...
        if not isinstance(observed, str) or not hash_re.fullmatch(observed):
            raise _CardContractError("%s source_hash must be 12 lowercase hex" % relative)
        ...
        if body_bytes > budget["max_body_bytes"]:
            raise _CardContractError("%s body has %d bytes; budget is %d" %
                                    (relative, body_bytes,
                                     budget["max_body_bytes"]))
        if action_items > budget["max_action_items"]:
            raise _CardContractError("%s has %d action items; budget is %d" %
                                    (relative, action_items,
                                     budget["max_action_items"]))
        if list(_heading_sequence(text)) != schema["body_sections"]:
            raise _CardContractError("%s sections must be exactly %s" %
                                    (relative, schema["body_sections"]))
        match = PROHIBITED_BODY_RE.search(body)
        if match:
            raise _CardContractError("%s contains implementation/compiled prose: %s" %
    ...
    if not records:
        raise _CardContractError("canonical Card directory has no Cards")
...
def main(argv=None):
    ...
    if not root.is_dir():
        ...
        return 1
    try:
        cards, read_sets = discover_cards(root)
    except (_CardContractError,
    ...
    for route_id in sorted(cards):
        ...
        except _CardContractError as exc:
            print("stamp_cards: FAIL — %s" % exc)
            return 1
        reasons = []
    ...
    try:
        card_schema = _load_card_schema(root)
        read_set_schema = read_set_contract.load_schema(root)
    except (_CardContractError,
            read_set_contract.ReadSetContractError) as exc:
        print("stamp_cards: FAIL — %s" % exc)

...
def _closed_string_list(value, label, errors):
    if (not isinstance(value, list) or not value or
            not all(_nonempty_string(item) and item.strip() == item
                    for item in value)):
...
def _closed_marker_mapping(value, expected_keys, label, errors):
    if not isinstance(value, dict) or set(value) != set(expected_keys):
        ...
        return {}
    if not all(_nonempty_string(item) and item.strip() == item
...
def parse_control_registry_document(document, *, audit_values=None,
    ...
    for index, row in enumerate(gate_rows):
        ...
        if not all(_nonempty_string(row.get(field))
...
def parse_legacy_standards_gate_registry_markdown(text):
    """Parse archived K00/12 Gate-table bytes for historical replay only."""
    errors = []
    registry = {}
    if not isinstance(text, str):
        return {}, ["legacy Gate registry bytes must decode as text"]
    inside = False
    seen = 0
    for line in text.splitlines():
        heading = re.match(r"^(#{2,3})\s+(.*?)\s*#*\s*$", line)
        if heading:
            active = heading.group(2).strip() == "Stable Gate ID Registry"
            if active:
                seen += 1
            inside = active and seen == 1
            continue
        ...
        if cells == ["Gate ID", "Tool", "Tool version", "Check", "Mode",
                     "Dimension", "Lifecycle"] or \
                (cells and all(re.fullmatch(r":?-+:?", cell)
                               for cell in cells)):
            continue
        if len(cells) != 7:
            errors.append("legacy Stable Gate row must have seven cells")
            continue
        gate_id, tool, version, check, mode, dimensions, lifecycle = cells
        if gate_id in registry:
            errors.append("legacy Stable Gate registry repeats %s" % gate_id)
            continue
        registry[gate_id] = {
            "tool": tool,
            ...
            "dimensions": tuple(sorted(
                ...
                re.split(r"[,\s]+", dimensions) if token.strip())),
            "lifecycle_states": tuple(sorted(
                token.strip().strip("`") for token in
                re.split(r"[,\s]+", lifecycle) if token.strip())),
        }
    if seen != 1:
        errors.append(
            "legacy K00/12 must contain exactly one Stable Gate ID Registry")
    if not registry:
        errors.append("legacy Stable Gate ID Registry has no rows")
    return registry, errors

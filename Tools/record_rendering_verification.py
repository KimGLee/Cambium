...
TOOL_VERSION = "1.0.0"
CHECK = "changed_scope_rendering_escalation_record"
...
def resolve_obligation(plan, obligation_id, contract=None):
    ...
    projected_fields = (
        ...
        "applicability", "partition", "due_stage", "producer_check",
    ...
    expected = {field: projection[field] for field in projected_fields}
    expected.update({
...
def build_record(*, root, plan, plan_sha256, obligation, frozen,
    ...
    resolved = resolve_obligation(plan, obligation["obligation_id"], contract)
        ...
        audit_producer_runtime.obligation_contract_fingerprint(
            plan, obligation, additional={
                "rendering_verification_contract_sha256":
                    rendering_verification_contract.contract_sha256(contract),
            })
    identity = {
...
def _context(root_arg, batch_id, plan_path, obligation_id):
    ...
    obligation = resolve_obligation(
        stage["plan"], obligation_id, contract)
    frozen = audit_producer_runtime.freeze_manifest_pages(root, result, item)
    _reject_existing(result, stage["plan"], obligation)
    return {

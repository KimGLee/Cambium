...
def nonempty_string(value, label):
    """Return one validated non-empty string used by machine contracts."""
...
def field_specs(value, label, *, allowed_types):
    ...
    for index, row in enumerate(value):
        ...
        field = nonempty_string(row.get("field"), item_label + ".field")
        if field in specs:
...
def validate_contract(document, *, root=None, snapshots=None):
    ...
    if document.get("schema_version") != 1:
        raise ValueError("AuditPlan contract schema_version must be 1")
    ...
    required_plan_fields = {
        "schema_version", "plan_id", "task_id", "batch_id", "generated_at",
        ...
        "profile_contract_fingerprint", "opening_transition_receipt",
        "artifact_snapshot_sha256",
        "contract_snapshot_sha256", "accepted_baseline_sha256",
        "obligations",
    }
    required_obligation_fields = {
        "obligation_id", "owner_kind", "owner_rule_id",
        "kernel_extension_point", "partition", "due_stage", "target",
        ...
        "review_due", "status", "evidence_ref", "reused_receipt_id",
        "reuse_reason",
    }
    if set(fields) != required_plan_fields:
        raise ValueError("AuditPlan instance fields do not match K12/19")
    if set(obligation_fields) != required_obligation_fields:
        raise ValueError("AuditPlan obligation fields do not match K12/19")
    expected_partitions = {
        "mandatory-full-deterministic", "changed-scope-deterministic",
        "initial-semantic-review", "invalidated-semantic-review",
        "overdue-targeted-review", "bounded-sampling", "reusable-evidence",
        "profile-registered-review",
    }
    if set(partitions) != expected_partitions:
        raise ValueError("AuditPlan partition namespace does not match K12/19")
    if set(statuses) != {"required", "reused"}:
        raise ValueError("AuditPlan obligation statuses must be required/reused")
    if set(due_stages) != {"pre-merge", "post-delta-close"}:
        raise ValueError("AuditPlan due-stage namespace is not closed")
    if set(owner_kinds) != {"kernel", "profile-extension"}:
        raise ValueError("AuditPlan owner-kind namespace is not closed")
    if set(fingerprint_bindings) != {"evidence-time", "reused-receipt"}:
        raise ValueError("AuditPlan fingerprint binding namespace is not closed")
    return {
...
def validate_plan(plan, contract=None, dimensions=None):
    ...
    if plan.get("schema_version") != 1:
        raise ValueError("AuditPlan schema_version must be 1")
...
__all__ = [
    "AUDIT_DUE_STAGES", "AUDIT_EVIDENCE_KINDS",
    "AUDIT_PLAN_CONTRACT_PATH", "OBLIGATION_FIELDS",
    "OBLIGATION_STATUS_VALUES", "PARTITION_IDS", "PLAN_FIELDS",
    "closed_string_list", "field_specs", "is_sha256", "load_contract",
    "nonempty_string", "plan_sha256", "required_obligation_ids",
    "validate_contract", "validate_plan", "validate_value",
]
